...
LOCAL_DEBUG_OUT("Incoming message in proto 2%s","");
			...
			LOCAL_DEBUG_OUT( "name_size = %d, pfunc->name = %p", ibuf->name_size, pfunc->name );
			if (res > 0)
			...
			LOCAL_DEBUG_OUT( "text_size = %d, pfunc->text = %p", ibuf->text_size, pfunc->text );
			if (res > 0)
			{	
...
PositiveWrite (unsigned int channel, send_data_type *ptr, int size)
{
	...
	LOCAL_DEBUG_OUT( "module(%p)->active(%d)->module_mask(0x%X)->mask(0x%X)", module, module->active, module->mask, mask );
...
ExecModule (char *action, Window win, int context)
{
	...
	args = stripcpy( args );
	spawn_child( cmd, -1, Scr.screen, NULL, win, context, True, True, args, NULL );
