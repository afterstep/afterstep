	 ...
	 case F_MOVECURSOR:
		 ...
		 if ((delta_x != 0) || (delta_y != 0))
		 {
			 ...
			 XWarpPointer (dpy, Scr.Root, Scr.Root, 0, 0, Scr.MyDisplayWidth,
						   Scr.MyDisplayHeight, x - warp_x, y - warp_y);
		 }
#endif
		 XWarpPointer (dpy, Scr.Root, Scr.Root, 0, 0, Scr.MyDisplayWidth,
...
/* If + deiconify, AfterStep crashes X ! */
	 ...
	 case F_REFRESH:
		 {
			 ...
			 w = create_visual_window (Scr.asv, Scr.Root, 0, 0,
									   ...
									   (unsigned int)0, InputOutput, valuemask, &attributes);
			 XMapWindow (dpy, w);
	 ...
	 case F_MODULE:
		 if (eventp->type != KeyPress)
			 UngrabEm ();
		 executeModule (action, (FILE *) NULL, (char **)((tmp_win) ? tmp_win->w : 0),
						(int *)context);
		...
		if (((func == F_WARP_F) &&
			 (((current_win != NULL) && (t->warp_index > look_for_warp) &&
			   (t->warp_index <= selected_warp)) || ((current_win == NULL) &&
													 (t->warp_index >= look_for_warp) &&
													 (t->warp_index < selected_warp)))) ||
			((func == F_WARP_B) &&
			 (((current_win != NULL) && (t->warp_index < look_for_warp) &&
			   (t->warp_index >= selected_warp)) || ((current_win == NULL) &&
													 (t->warp_index >= look_for_warp) &&
			...
			if (tmp_win)
			{
				if (t != tmp_win &&
					((direction == DOWN &&
					 ...
					 (direction == UP &&
					  t->circulate_sequence > tmp_win->circulate_sequence &&
					  (target == NULL || t->circulate_sequence < target->circulate_sequence))))
					target = t;
			} else if (t != target &&
					   ((direction == DOWN &&
						 (target == NULL ||
 ...
 ***********************************************************************/
void
Maximize (ASWindow * tmp_win, int val1, int val2, int val1_unit, int val2_unit)
{
...
#endif /* ! NO_SHADE */
		XResizeWindow (dpy, tmp_win->frame, tmp_win->frame_width, tmp_win->frame_height);
/*
 ...
 */
	...
	} else
	{
		...
		XResizeWindow (dpy, tmp_win->frame, tmp_win->title_width, tmp_win->title_height);
 ...
 */
		if ((buf != NULL) && (length > 0))
		{
			...
			XQueryPointer (dpy, Scr.Focus->w, &event.xkey.root, &event.xkey.subwindow,
						   &event.xkey.x_root, &event.xkey.y_root, &event.xkey.x, &event.xkey.y,
						   &event.xkey.state);
			for (i = 0; i < length; i++)
			{
...
GrabEm (int cursor)
{
	...
	while ((i < 1000) && (val = XGrabPointer (dpy, Scr.Root, True, mask,
											  ...
											  Scr.ASCursors[cursor], CurrentTime) != GrabSuccess))
	{
		i++;
...
IsClick (int x, int y, unsigned EndMask, XEvent * d)
{
	...
	while ((total < Scr.ClickTime) &&
		   (x - xcurrent < 5) && (x - xcurrent > -5) && (y - ycurrent < 5) && (y - ycurrent > -5))
	{
				 ...
				 unsigned long context, MenuRoot * mr)
{
	...
	while (mi != NULL)
	{
		...
		else if (c == IMMEDIATE)
		{
			...
			ExecuteFunction (mi->func, mi->action, w,
							 ...
							 mi->val1_unit, mi->val2_unit, mi->menu, -1);
		} else
			Persist = True;
		mi = mi->next;
	}
	...
	if ((HaveTripleClick) && (type == DOUBLE_CLICK) && (IsClick (x, y, ButtonPressMask, &d)))
	{
	...
	while (mi != NULL)
	{
		...
		if (c == type)
		{
			...
			ExecuteFunction (mi->func, mi->action, w,
							 ...
							 mi->val1, mi->val2, mi->val1_unit, mi->val2_unit, mi->menu, -2);
		}
		mi = mi->next;
	}
	WaitForButtonsUp ();
