...
RestoreWithdrawnLocation (ASWindow * tmp, Bool restart)
{
  ...
  if (XGetGeometry (dpy, tmp->w, &JunkRoot, &JunkX, &JunkY,
    ...
    {
      get_client_geometry (tmp, tmp->frame_x, tmp->frame_y, tmp->frame_width, tmp->frame_height, &xwc.x, &xwc.y, NULL, NULL);
...
void
MyXUngrabButton (Display * display, unsigned button, unsigned modifiers,
		 Window grab_window)
{
  unsigned mod, *mods;
  mods = (modifiers != AnyModifier) ? Scr.lock_mods : mygrabs_no_mods;
