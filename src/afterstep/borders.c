...
SetBackgroundTexture (ASWindow * t, Window win, MyStyle * style, Pixmap cache)
{
	...
	else if (style->texture_type < TEXTURE_PIXMAP)
	{
		...
		if (cache == None)
		{
			...
			XGetGeometry (dpy, win, &JunkRoot, &JunkX, &JunkY, &width, &height, &JunkBW,
						  &JunkDepth);
...
SetTransparency (ASWindow * t)
{
	...
	if (t->style_focus->texture_type >= TEXTURE_TRANSPARENT)
	{
		...
		t->backPixmap =
			mystyle_make_pixmap_overlay (t->style_focus, x, y, t->title_width, t->title_height,
										 None);
		updated = 1;
	}
	if (t->style_unfocus->texture_type >= TEXTURE_TRANSPARENT)
	{
		...
		t->backPixmap2 =
			mystyle_make_pixmap_overlay (t->style_unfocus, x, y, t->title_width, t->title_height,
										 None);
		updated = 1;
	}
	if (t->style_sticky->texture_type >= TEXTURE_TRANSPARENT)
	{
		...
		t->backPixmap3 =
			mystyle_make_pixmap_overlay (t->style_sticky, x, y, t->title_width, t->title_height,
										 None);
		updated = 1;
	}
#endif /* !NO_TEXTURE */
 ...
 ****************************************************************************/
void
SetBorder (ASWindow * t, Bool is_focus, Bool force, Bool Mapped, Window expose_win)
{
	int           i, x;
...
/* icons are handled separately */
	...
	if (t->flags & BORDER)
	{
		...
		RelieveWindow (t, t->side, 0, 0, x, t->boundary_height - t->bw, ReliefGC, ShadowGC, 0x0004);
		for (i = 0; i < 2; i++)
		{
...
/* figure out where the title text goes */
	if (t->flags & VERTICAL_TITLE)
		switch (Scr.TitleTextAlign)
		{
		 ...
		 case JUSTIFY_RIGHT:
			 text_y =
				 (*style).font.y + t->title_height - (text_w + t->space_taken_right_buttons + 6);
			 break;
		 case JUSTIFY_CENTER:
		 default:
			 text_y =
				 (*style).font.y + (t->title_height - t->space_taken_left_buttons -
									t->space_taken_right_buttons - text_w) / 2 +
				 t->space_taken_left_buttons;
			 break;
	} else
		switch (Scr.TitleTextAlign)
	...
	} else
	{
		...
		if (t->name != NULL)
		{
			...
			if (t->flags & VERTICAL_TITLE)
				...
				mystyle_draw_texturized_text (t->title_w, style, text_texture_style, txt, text_x,
											  text_y);
		}
...
RelieveWindow (ASWindow * t, Window win,
			   int x, int y, int w, int h, GC ReliefGC, GC ShadowGC, int hilite)
{
	XSegment      seg[4];
	int           i;
	int           edge;
 ...
 */
	...
	get_parent_geometry (tmp_win, tmp_win->frame_width, tmp_win->frame_height, &cx, &cy,
	...
	get_window_geometry (tmp_win, tmp_win->flags & ~ICONIFIED, &frame_wc.x, &frame_wc.y,
	...
	if ((tmp_win->flags & TITLE) && tmp_win->title_width > 2 && tmp_win->title_height > 2)
	{
		...
		if (tmp_win->style_focus->texture_type > 0 &&
			tmp_win->style_focus->texture_type < TEXTURE_PIXMAP && (tmp_win->backPixmap == None ||
																	tmp_win->bp_width !=
																	tmp_win->title_width ||
																	tmp_win->bp_height !=
																	tmp_win->title_height))
		{
			...
			tmp_win->backPixmap =
				mystyle_make_pixmap (tmp_win->style_focus, tmp_win->title_width,
									 tmp_win->title_height, None);
		}
		/* unfocused titlebar */
		if (tmp_win->style_unfocus->texture_type > 0 &&
			tmp_win->style_unfocus->texture_type < TEXTURE_PIXMAP && (tmp_win->backPixmap2 == None
																	  || tmp_win->bp_width !=
																	  tmp_win->title_width ||
																	  tmp_win->bp_height !=
																	  tmp_win->title_height))
		{
			...
			tmp_win->backPixmap2 =
				mystyle_make_pixmap (tmp_win->style_unfocus, tmp_win->title_width,
									 tmp_win->title_height, None);
		}
		/* unfocused sticky titlebar */
		if (tmp_win->style_sticky->texture_type > 0 &&
			tmp_win->style_sticky->texture_type < TEXTURE_PIXMAP && (tmp_win->backPixmap3 == None ||
																	 tmp_win->bp_width !=
																	 tmp_win->title_width ||
																	 tmp_win->bp_height !=
																	 tmp_win->title_height))
		{
			...
			tmp_win->backPixmap3 =
				mystyle_make_pixmap (tmp_win->style_sticky, tmp_win->title_width,
									 tmp_win->title_height, None);
		}
	}
...
void
get_parent_geometry (ASWindow * t, int frame_width, int frame_height, int *parent_x_out,
					 int *parent_y_out, int *parent_width_out, int *parent_height_out)
{
...
void
get_client_geometry (ASWindow * t, int frame_x, int frame_y, int frame_width, int frame_height,
					 int *client_x_out, int *client_y_out, int *client_width_out,
					 int *client_height_out)
{
	int           grav_x, grav_y;
...
void
get_frame_geometry (ASWindow * t, int client_x, int client_y, int client_width, int client_height,
					int *frame_x_out, int *frame_y_out, int *frame_width_out, int *frame_height_out)
{
	int           grav_x, grav_y;
 ...
 */
void
get_resize_geometry (ASWindow * t, int client_x, int client_y, int client_width, int client_height,
					 int *frame_x_out, int *frame_y_out, int *frame_width_out,
