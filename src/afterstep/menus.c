...
set_asmenu_item_data( ASMenuItem *item, MenuDataItem *mdi )
{
	...
	if (mdi->minipixmap[MINIPIXMAP_Icon].image)
		...
		icon_im = GetASImageFromFile( mdi->minipixmap[MINIPIXMAP_Icon].filename );
	...
	if( icon_im )
	{
		...
		if( item->icon != icon_im && icon_im != mdi->minipixmap[MINIPIXMAP_Icon].image )
...
LOCAL_DEBUG_CALLER_OUT( "%p,%d", menu, selection );
	...
	if( menu->items[selection].source )	{
		...
		if( menu->items[selection].source->comment ) 
			...
			balloon_delay = MenuBalloons->look->Delay > 1000 ? MenuBalloons->look->Delay-1000 : MenuBalloons->look->Delay;
		
		if (balloon_delay >= 0) {
			if( menu->item_balloon == NULL )
				menu->item_balloon = create_asballoon_for_state ( MenuBalloons, NULL);
		
			timer_new (balloon_delay, &menu_item_balloon_timer_handler, (void *)menu);
		}			
	}
}
...
menu_item_balloon_timer_handler (void *data)
{
	...
	if (menu && menu->item_balloon) {
		...
		if (item->source->comment)	{
			int	encoding = get_flags( item->source->flags, MD_CommentIsUTF8)? AS_Text_UTF8 : AS_Text_ASCII ;
