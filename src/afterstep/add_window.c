...
*/
  ...
  if (!(nl.off_flags & STYLE_FOCUS_FLAG) || (tmp_win->style_focus = mystyle_find (nl.style_focus)) == NULL)
    tmp_win->style_focus = Scr.MSFWindow;
  if (!(nl.off_flags & STYLE_UNFOCUS_FLAG) || (tmp_win->style_unfocus = mystyle_find (nl.style_unfocus)) == NULL)
    tmp_win->style_unfocus = Scr.MSUWindow;
  if (!(nl.off_flags & STYLE_STICKY_FLAG) || (tmp_win->style_sticky = mystyle_find (nl.style_sticky)) == NULL)
...
/*  if (!(nl.off_flags & NOFRAME_FLAG))
  ...
  get_frame_geometry (tmp_win, tmp_win->attr.x, tmp_win->attr.y, tmp_win->attr.width, tmp_win->attr.height, &tmp_win->frame_x, &tmp_win->frame_y, &tmp_win->frame_width, &tmp_win->frame_height);
