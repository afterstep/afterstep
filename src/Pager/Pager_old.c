...
//  set_output_threshold(OUTPUT_DEFAULT_THRESHOLD-1);
    ...
    LOCAL_DEBUG_OUT("Initializing desks ...");
    for (i = 0; i < Pager.ndesks; i++)
        InitDesk (i);

    if( (Pipes.x_fd = ConnectX( &Scr, MyArgs.display_name, PropertyChangeMask )) < 0 )
    {
        show_error("failed to initialize window manager session. Aborting!", Scr.screen);
        exit(1);
    }
    if (fcntl (Pipes.x_fd, F_SETFD, 1) == -1)
	{
    ...
    /* open a pager window */
    LOCAL_DEBUG_OUT("initializing windows ...");
    initialize_pager ();
...
{
  LOCAL_DEBUG_OUT("received AfterStep message #%ld, body[0]=%lu", type, body[0]);
    switch (type)
    {
    case M_ADD_WINDOW:
      list_configure (body);
...
list_configure (unsigned long *body)
{
    ...
    else
    {
        ...
        if (t->desk == body[7])
        {
            LOCAL_DEBUG_OUT("MoveResizePagerView of [%s] from list_configure: desk = %d", t->icon_name, t->desk);
            MoveResizePagerView (t);
        }else
        {
            LOCAL_DEBUG_OUT("ChangeDeskForWindow of [%s] from list_configure: from %d to %ld", t->icon_name, t->desk, body[7]);
...
list_new_page (unsigned long *body)
{
    ...
    if (body[2] != 10000)
    {
        LOCAL_DEBUG_OUT("list_new_page: desk = %d, Vx=%d, Vy=%d", Scr.CurrentDesk, Scr.Vx, Scr.Vy);
        MovePage ();
        MoveStickyWindows ();
    }
}
...
list_new_desk (unsigned long *body)
{
    ...
    if (body[0] != 10000)
    {
        LOCAL_DEBUG_OUT("list_new_desk(New=%d, Old=%d):\t", Scr.CurrentDesk, oldDesk);
    ...
    LOCAL_DEBUG_OUT("list_new_desk():\tDone!");
}
...
GetLook (FILE * fd)
{
  ...
  for (i = 0; i < STYLE_MAX_STYLE; i++)
	...
	Style = mystyle_find (PagerDefaultStyleNames[i]);
      LOCAL_DEBUG_OUT("%s Style is %s", buf, Style ? "Found" : "NotFound");
	if (!Style)
	{
