...
void CheckConfigSanity()
{
	...
	if (real_x + (int) Config->geometry.width < 0)
	    Config->geometry.x =
		get_flags(Config->geometry.flags,
			  XNegative) ? (int) Config->geometry.width -
		(int) Scr.MyDisplayWidth : 0;
	else if (real_x > (int) Scr.MyDisplayWidth)
	    Config->geometry.x =
		get_flags(Config->geometry.flags,
			  XNegative) ? 0 : (int) Scr.MyDisplayWidth -
		(int) Config->geometry.width;
    }
	...
	if (real_y + (int) Config->geometry.height < 0)
	    Config->geometry.y =
		get_flags(Config->geometry.flags,
			  YNegative) ? (int) Config->geometry.height -
		(int) Scr.MyDisplayHeight : 0;
	else if (real_y > (int) Scr.MyDisplayHeight)
	    Config->geometry.y =
		get_flags(Config->geometry.flags,
			  YNegative) ? 0 : (int) Scr.MyDisplayHeight -
		(int) Config->geometry.height;
    }
    ...
    LOCAL_DEBUG_OUT("active_bevel = %lX, inactive_bevel = %lX",
		    Config->active_desk_bevel,
		    Config->inactive_desk_bevel);
    mystyle_get_property(Scr.wmprops);

    for (i = 0; i < BACK_STYLES; ++i) {
	static char *window_style_names[BACK_STYLES] =
	    { "*%sFWindowStyle", "*%sUWindowStyle", "*%sSWindowStyle",
NULL };
	static char *default_window_style_name[BACK_STYLES] =
	    { "focused_window_style", "unfocused_window_style",
	...
	if (load_font(Config->small_font_name, &small_font)) {
		...
		if (Scr.Look.MSWindow[i])
		    mystyle_merge_font(Scr.Look.MSWindow[i], &small_font,
				       True);
	}
	...
	LOCAL_DEBUG_OUT
	    ("desk_style %d: \"%s\" ->%p(\"%s\")->colors(%lX,%lX)", i, buf,
	     Config->MSDeskTitle[i], Config->MSDeskTitle[i]->name,
 ...
 */
    ...
    Config->flags =
	(config->flags & config->
	 set_flags) | (Config->flags & (~config->set_flags));
	...
	if (Config->labels == NULL)
	    Config->labels =
		safecalloc(PagerState.desks_num, sizeof(char *));
	for (i = 0; i < PagerState.desks_num; ++i)
	    if (config->labels[i])
		set_string(&(Config->labels[i]),
			   mystrdup(config->labels[i]));
    }
    if (config->styles) {
	...
	for (i = 0; i < PagerState.desks_num; ++i)
	    if (config->styles[i])
		set_string(&(Config->styles[i]),
			   mystrdup(config->styles[i]));
    }
	...
	set_string(&(Config->border_color),
		   mystrdup(config->border_color));
...
Window make_pager_window()
{
    ...
    set_client_hints(w, NULL, &shints, AS_DoesWmDeleteWindow,
		     &extwm_hints);
    set_client_cmd(w);
...
void update_pager_shape()
{
	...
	if (Config->MSDeskBack[i]->texture_type == TEXTURE_SHAPED_PIXMAP ||
		...
		add_shape_rectangles(PagerState.main_canvas->shape,
				     &(PagerState.selection_bar_rects[0]),
				     4, x, y,
		...
		int k = d->clients_num;
		LOCAL_DEBUG_OUT("desk %d clients_num %d", i,
				d->clients_num);
		while (--k >= 0) {
			...
			unsigned int client_width, client_height,
			    client_bw;
			get_current_canvas_geometry(clients[k]->canvas,
						    &client_x, &client_y,
						    &client_width,
						    &client_height,
						    &client_bw);
			...
			LOCAL_DEBUG_OUT("combining client \"%s\"",
					clients[k]->icon_name);
			combine_canvas_shape_at_geom(PagerState.
						     main_canvas,
						     clients[k]->canvas,
						     client_x + x,
						     client_y + y,
...
void
place_desk(ASPagerDesk * d, int x, int y, unsigned int width,
	   unsigned int height)
{
    int win_x = 0, win_y = 0;
    unsigned int bw;
...
void restack_desk_windows(ASPagerDesk * d)
{
	...
	while (++i < d->clients_num) {
		...
		LOCAL_DEBUG_OUT
		    ("k = %d, w = %lX, client = %lX, wd = %p, name = \"%s\"",
		     k, clients[i]->canvas->w, clients[i]->client,
		     clients[i],
		     clients[i]->window_name ? clients[i]->
		     window_name : "(null)");
...
void place_selection()
{
	...
	if (sel_desk != NULL && sel_desk->background
	    && sel_desk->desk_canvas) {
		...
		while (--i >= 0)
		    XMoveResizeWindow(dpy, PagerState.selection_bars[i],
				      ...
				      PagerState.selection_bar_rects[i].
				      width,
				      PagerState.selection_bar_rects[i].
				      height);
	    }
...
void redecorate_pager_desks()
{
	...
	ARGB2PIXEL(Scr.asv, Config->border_color_argb,
		   &(attr.border_pixel));
	if (d->desk_canvas == NULL) {
	    ...
	    w = create_visual_window(Scr.asv, PagerState.main_canvas->w, 0,
				     0, PagerState.desk_width,
	...
	if (get_flags(Config->flags, USE_LABEL)) {
		...
		get_flags(Config->flags,
			  VERTICAL_LABEL) ? FLIP_VERTICAL : 0;
		...
		add_astbar_btnblock(d->title, flip ? 0 : 1, 0, flip,
				    NO_ALIGN, &list[0], 0xFFFFFFFF, 1,
				    PagerState.tbar_props->
				    buttons_h_border,
				    PagerState.tbar_props->
				    buttons_v_border,
				    PagerState.tbar_props->buttons_spacing,
				    0);
	    }
	    if (get_flags(Config->flags, VERTICAL_LABEL)) {
		...
		LOCAL_DEBUG_OUT("size = %d, max_title_height = %d", size,
				max_title_height);
		d->title_width =
		    PagerState.desk_width - Config->border_width * 2;
	...
	if (get_flags(Config->flags, PAGE_SEPARATOR)) {
	    ...
	    for (p = 0; p < d->separator_bars_num; ++p) {
		d->separator_bars[p] =
		    create_visual_window(Scr.asv, d->desk_canvas->w, 0, 0,
	...
	for (i = 0; i < 4; i++)
		...
		PagerState.selection_bars[i] =
		    create_visual_window(Scr.asv,
					 PagerState.main_canvas->w, 0, 0,
					 1, 1, 0, InputOutput, CWBackPixel,
					 &attr);
	    } else {
...
void rearrange_pager_desks(Bool dont_resize_main)
{
	...
	for (i = 0; i < PagerState.desks_num; ++i) {
		...
		(" :RESIZING: desk = %d, size = %dx%d, all_size = %+d%+d, +x+y = %+d%+d, row_height = %d",
		 i, width, height, all_width, all_height, x, y,
		 row_height);
...
{
    int main_height =
	PagerState.main_canvas->height - Config->border_width;
    if (get_flags(Config->flags, VERTICAL_LABEL))
	     ...
	     Bool dont_update_shape)
{
	...
	if (get_flags(wd->state_flags, AS_Iconic)) {
		...
		if (get_flags(wd->flags, AS_VerticalTitle))
		    client_width =
			(PagerState.vscreen_width * 2) / desk_width;
		else
		    client_height =
			(PagerState.vscreen_height * 2) / desk_height;
	    }
	}
	...
	if (wd->canvas) {
	    ...
	    LOCAL_DEBUG_OUT("+PLACE->canvas(%p)->geom(%dx%d%+d%+d)",
			    wd->canvas, width, height, x, y);
	}
    }
}
...
void set_client_look(ASWindowData * wd, Bool redraw)
{
    ...
    if (wd->bar) {
	int state =
	    get_flags(wd->state_flags,
		      AS_Sticky) ? BACK_STICKY : BACK_UNFOCUSED;
...
void add_client(ASWindowData * wd)
{
    ...
    set_astbar_hilite(wd->bar, BAR_STATE_FOCUSED,
		      NORMAL_HILITE | NO_HILITE_OUTLINE);
    add_astbar_label(wd->bar, 0, 0, 0, NO_ALIGN, 0, 0, NULL,
		     AS_Text_ASCII);
...
change_desk_stacking(int desk, unsigned int clients_num, Window * clients)
{
	...
	if (wd != NULL) {	/* window is in stacking order, but wew were not notifyed about it yet */
		...
		d->clients[i] = wd;
		++real_clients_count;
		LOCAL_DEBUG_OUT("id(%lX)->wd(%p)", clients[i],
				d->clients[i]);
	    }
	}
    }
...
void move_sticky_clients()
{
	...
	if (d->clients && d->clients_num > 0) {
	    ...
	    while (--i >= 0)
		if (clients[i]
		    && get_flags(clients[i]->state_flags, AS_Sticky)) {
			...
			LOCAL_DEBUG_OUT("reparenting client to desk %ld",
					d->desk);
			quietly_reparent_canvas(clients[i]->canvas,
						current_desk->desk_canvas->
						w, CLIENT_EVENT_MASK,
						False, None);
		    }
				       ...
				       int *ret_x, int *ret_y)
{
	...
	&& x <
	PagerState.main_canvas->width + PagerState.main_canvas->root_x
	&& y <
	...
	while (--i >= 0 && d == NULL) {
	    ...
	    LOCAL_DEBUG_OUT
		("checking desk %d: pos(%+d%+d)->desk_geom(%dx%d%+d%+d)",
		 i, x, y, d->desk_canvas->width, d->desk_canvas->height,
		      ...
		      unsigned int *ret_width, unsigned int *ret_height)
{
	...
	if (d->background->width > 0) {
	    width =
		(width * PagerState.vscreen_width) / d->background->width;
	}
	if (d->background->height > 0) {
	    height =
		(height * PagerState.vscreen_height) /
		d->background->height;
	}
	*ret_width = width;
	*ret_height = height;
...
void exec_moveresize_req(void *data)
{
	...
	SendNumCommand(req->func, NULL, &(req->func_val[0]),
		       &(unit_val[0]), req->client);
	req->pending = False;
    }
}
...
ASGrid *make_pager_grid()
{
	...
	add_gridline(&(grid->h_lines), bb->root_y, bb->root_x,
		     bb->root_x + bb->width, resist, attract);
	add_gridline(&(grid->h_lines), bb->root_y + bb->height, bb->root_x,
		     bb->root_x + bb->width, resist, attract);
	add_gridline(&(grid->v_lines), bb->root_x, bb->root_y,
		     bb->root_y + bb->height, resist, attract);
	add_gridline(&(grid->v_lines), bb->root_x + bb->width, bb->root_y,
		     bb->root_y + bb->height, resist, attract);
	...
	if (!get_flags(d->flags, ASP_DeskShaded)) {	/* add all the grid separation windows : */
	    ...
	    /* vertical bars : */
	    while (--p >= 0) {
		add_gridline(&(grid->v_lines), pos, pos2, pos2 + size,
			     resist, attract);
		pos -= pos_inc;
	    }
	    /* horizontal bars */
	    ...
	    size = bb->width;
	    while (--p >= 0) {
		add_gridline(&(grid->h_lines), pos, pos2, pos2 + size,
			     resist, attract);
		pos -= pos_inc;
	    }
	}
	while (--k >= 0)
		...
		if (inner_gravity != 0)
		    add_canvas_grid(grid, wd->canvas, outer_gravity,
				    inner_gravity, 0, 0);
	    }
    }
...
/*         show_activity( "message %lX window %X data %p", type, body[0], wd ); */
	...
	if (res == WP_DataCreated && get_flags(wd->flags, AS_HitPager))
	    add_client(wd);
		 ...
		 && get_flags(wd->flags, AS_HitPager)) {
	...
	if (!get_flags(PagerState.flags, ASP_ReceivingWindowList)) {
		...
		register INT32 pager_desk =
		    saved_desk - PagerState.start_desk;
		if (pager_desk >= 0 && pager_desk < PagerState.desks_num)
		...
		register INT32 pager_desk =
		    new_desk - PagerState.start_desk;
		if (pager_desk >= 0 && pager_desk < PagerState.desks_num)
		    if (Config->MSDeskBack[pager_desk]->texture_type ==
			TEXTURE_SHAPED_PIXMAP
			|| Config->MSDeskBack[pager_desk]->texture_type ==
	...
	case M_NEW_DESKVIEWPORT:
	    {
		LOCAL_DEBUG_OUT
		    ("M_NEW_DESKVIEWPORT(desk = %ld,Vx=%ld,Vy=%ld)",
	...
	case M_STACKING_ORDER:
	    {
		LOCAL_DEBUG_OUT
		    ("M_STACKING_ORDER(desk=%ld, clients_num=%ld)",
		     body[0], body[1]);
...
void DispatchEvent(ASEvent * event)
{
    ...
    if (Scr.moveresize_in_progress) {
	event->widget =
	    PagerState.resize_desk ? PagerState.
	    resize_desk->desk_canvas : PagerState.main_canvas;
	...
	if (wd) {
		...
		if ((event->eclass & ASE_POINTER_EVENTS) != 0) {
		    on_astbar_pointer_action(((ASWindowData
					       *) (event->client))->bar, 0,
					     (event->x.type ==
					      LeaveNotify),
					     root_pointer_moved);
	...
	if (event->client != NULL) {
	    ASWindowData *wd = (ASWindowData *) (event->client);
	    event->x.xkey.window = wd->client;
	    XSendEvent(dpy, wd->client, False, KeyReleaseMask,
		       &(event->x));
	}
	return;
    case ButtonPress:
	...
	if (event->x.xproperty.atom == _AS_BACKGROUND) {
		...
		update_astbar_transparency(PagerState.desks[i].title,
					   PagerState.desks[i].desk_canvas,
					   True);
		...
		register ASWindowData **clients =
		    PagerState.desks[i].clients;
		...
		while (--k >= 0) {
			...
			show_warning("client %d of the desk %d is NULL", k,
				     i);
		}
	    }
	    redecorate_pager_desks();
...
void
on_pager_window_moveresize(void *client, Window w, int x, int y,
			   unsigned int width, unsigned int height)
{
	...
	if (w == PagerState.main_canvas->w) {	/* need to rescale everything? maybe: */
		...
		LOCAL_DEBUG_OUT
		    (" :RESIZING:  old_desk_size(%dx%d) new_desk_size(%dx%d)",
		     PagerState.desk_width, PagerState.desk_height,
		     new_desk_width, new_desk_height);
...
void on_desk_pressure_changed(ASPagerDesk * d, ASEvent * event)
{
    ...
    LOCAL_DEBUG_OUT
	("root_pos(%+d%+d)->title_root_pos(%+d%+d)->context(%s)", root_x,
	 root_y, d->title ? d->title->root_x : 0,
	 d->title ? d->title->root_y : 0, context2text(context));
	...
	set_astbar_pressed(d->background, d->desk_canvas,
			   context & C_TITLE);
...
void on_pager_pressure_changed(ASEvent * event)
{
	...
	if (event->w == PagerState.main_canvas->w) {
		...
		if (PagerState.desks[i].desk_canvas->w == event->w) {
		    on_desk_pressure_changed(&(PagerState.desks[i]),
					     event);
		    break;
		}
	}
	...
	start_moveresize_client((ASWindowData *) (event->client),
				(event->x.xbutton.state & ControlMask) ==
				0, event);
}
...
void release_pressure()
{
	...
	if (PagerState.pressed_desk) {
		...
		if (px > 0 && px < d->desk_canvas->width &&
		    ...
		    py -= d->background->win_y;
		    if (px >= 0 && py >= 0 &&
			px < d->background->width
			&& py < d->background->height) {
			if (PagerState.pressed_button == Button3) {
			    new_vx =
				(px * PagerState.vscreen_width) /
				d->background->width;
			    new_vy =
				(py * PagerState.vscreen_height) /
				d->background->height;
			    ...
			    new_vx =
				(px * PagerState.page_columns) /
				d->background->width;
			    new_vy =
				(py * PagerState.page_rows) /
				d->background->height;
	...
	set_astbar_pressed(PagerState.pressed_bar,
			   PagerState.pressed_canvas, False);
