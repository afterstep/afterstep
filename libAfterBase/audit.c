...
void count_alloc (const char *fname, int line, void *ptr, size_t length,
		  int type);
mem *count_find (const char *fname, int line, void *ptr, int type,
		 mem ** pm2);
mem *count_find_and_extract (const char *fname, int line, void *ptr,
			     int type);
...
count_find (const char *fname, int line, void *ptr, int type, mem ** pm2)
{
  ...
  /* scan for an invalid allocation list */
  {
    int q;
    mem *mmm;
    for (q = 0, mmm = first_mem; mmm != NULL && q < MAX_AUDIT_ALLOCS;
	 mmm = mmm->next, q++);
...
print_unfreed_mem (void)
{
  ...
  fprintf (stderr, "----------------------\n");
  fprintf (stderr,
      "allocating function    |line |length |pointer    |type (subtype)\n");
  fprintf (stderr,
      "-----------------------+-----+-------+-----------+--------------\n");
  for (q = 0, m = first_mem; m != NULL && q < MAX_AUDIT_ALLOCS;
       m = m->next, q++)
    if (m->freed == 0)
      {
	fprintf (stderr, "%23s|%-5d|%-7d|0x%08x ", m->fname, m->line,
		 m->length, (unsigned int) m->ptr);
	switch (m->type & 0xff)
	  {
	  case C_MEM:
		     ...
		     unsigned int height, unsigned int depth)
{
  ...
  if (pmap == None)
    return None;
  count_alloc (fname, line, (void *) pmap, width * height * depth / 8,
	       C_PIXMAP | C_CREATEPIXMAP);
  return pmap;
}
			     ...
			     unsigned int width, unsigned int height)
{
  ...
  if (pmap == None)
    return None;
  count_alloc (fname, line, (void *) pmap, width * height / 8,
	       C_PIXMAP | C_BITMAPFROMDATA);
  return pmap;
}
...
count_xcreatepixmapfrombitmapdata (const char *fname, int line,
				   ...
				   unsigned int height, unsigned long fg,
				   unsigned long bg, unsigned int depth)
{
  Pixmap pmap =
  XCreatePixmapFromBitmapData (display, drawable, data, width, height, fg,
			       bg, depth);
  if (pmap == None)
    return None;
  count_alloc (fname, line, (void *) pmap, width * height * depth / 8,
	       C_PIXMAP | C_FROMBITMAP);
  return pmap;
}

int
count_xfreepixmap (const char *fname, int line, Display * display,
		   Pixmap pmap)
{
...
count_xcreateimage (const char *fname, int line, Display * display,
		    ...
		    unsigned int height, int bitmap_pad, int byte_per_line)
{
  XImage *image =
		 ...
		 unsigned int height, unsigned long plane_mask, int format)
{
  ...
  count_alloc (fname, line, (void *) image,
	       sizeof (*image) + image->height * image->bytes_per_line,
	       C_IMAGE | C_GETIMAGE);
  return image;
}
...
count_xpmreadfiletopixmap (const char *fname, int line, Display * display,
			   Drawable drawable, char *filename, Pixmap * pmap,
			   Pixmap * mask, void *attributes)
{
  XpmAttributes *xpm_attributes = attributes;
  int val;
  val =
    XpmReadFileToPixmap (display, drawable, filename, pmap, mask,
			 xpm_attributes);
  if (pmap != NULL && *pmap != None)
	...
	size =
	  xpm_attributes->width * xpm_attributes->height *
				  ...
				  void *attributes)
{
  ...
  int val;
  val =
    XpmCreateImageFromXpmImage (display, xpm_image, image, mask,
				xpm_attributes);
...
count_xdestroyimage (const char *fname, int line, XImage * image)
{
  ...
  if ((m = count_find_and_extract (fname, line, (void *) image, C_IMAGE)) ==
      NULL)
    /* can also be of C_MEM type if we allocated it ourselvs */
			  ...
			  unsigned char **prop_return)
{
  ...
  val =
    XGetWindowProperty (display, w, property, long_offset, long_length,
			...
			actual_format_return, &my_nitems_return,
			bytes_after_return, prop_return);
  if (val == Success && my_nitems_return)
    count_alloc (fname, line, (void *) *prop_return,
		 my_nitems_return * *actual_format_return / 8,
		 C_XMEM | C_XGETWINDOWPROPERTY);
		  ...
		  Window ** children_return, unsigned int *nchildren_return)
{
  ...
  if (val && *nchildren_return)
    count_alloc (fname, line, (void *) *children_return,
		 *nchildren_return * sizeof (Window), C_XMEM | C_XQUERYTREE);
  return val;
}
...
count_xgetwmhints (const char *fname, int line, Display * display, Window w)
{
  ...
  if (val != NULL)
    count_alloc (fname, line, (void *) val, sizeof (XWMHints),
		 C_XMEM | C_XGETWMHINTS);
  return (void *) val;
}
		       ...
		       Window w, void *protocols_return, int *count_return)
{
  Status val;
  val =
    XGetWMProtocols (display, w, (Atom **) protocols_return, count_return);
  if (val && *count_return)
...
Status
count_xgetwmname (const char *fname, int line, Display * display, Window w,
		  void *text_prop_return)
{
  ...
  if (val && prop->nitems)
    count_alloc (fname, line, (void *) prop->value,
		 prop->nitems * prop->format / 8, C_XMEM | C_XGETWMNAME);
  return val;
}

/* class_hints_return is really XTextProperty*, but to avoid extra includes, we'll use void* */
Status
count_xgetclasshint (const char *fname, int line, Display * display, Window w,
		     void *class_hint_return)
{
  Status val;
  XClassHint *prop = class_hint_return;
  val = XGetClassHint (display, w, prop);
  if (val && prop->res_name)
    count_alloc (fname, line, (void *) prop->res_name,
		 strlen (prop->res_name), C_XMEM | C_XGETCLASSHINT);
  if (val && prop->res_class)
    count_alloc (fname, line, (void *) prop->res_class,
		 strlen (prop->res_class), C_XMEM | C_XGETCLASSHINT);
  return val;
}
...
count_xgetatomname (const char *fname, int line, Display * display, Atom atom)
{
  ...
  if (val != NULL)
    count_alloc (fname, line, (void *) val, strlen (val),
		 C_XMEM | C_XGETATOMNAME);
  return val;
}

/* text_prop_return is really XTextProperty*, but to avoid extra includes, we'll use void* */
Status
count_xstringlisttotextproperty (const char *fname, int line, char **list,
				 int count, void *text_prop_return)
{
  Status val;
  XTextProperty *prop = text_prop_return;
  ...
  if (val && prop->nitems)
    count_alloc (fname, line, (void *) prop->value,
		 ...
		 C_XMEM | C_XSTRINGLISTTOTEXTPROPERTY);
  return val;
}
