...
int event_traceble(XEvent *e)
{
  if( e )
    if( e->type >= KeyPress  && e->type <LASTEvent )
	if( event_types[e->type].mask & (EVENT_TRACE_MASK) ) return 1;
  return 0 ;
}

void printf_xevent(XEvent *e)
{
  if( e )
  {
    if( e->type >= KeyPress  && e->type <LASTEvent )
    {
        fprintf( stderr, "<%s>,%s,0x%lX,...", event_types[e->type].name, (e->xany.send_event)?"SendEvent":"NotSendEvent",e->xany.window );
    }else
    {
...
int func_traceble(int func)
{
    if( func >= 0 && func < F_FUNCTIONS_NUM )
	if(func_types[func].mask & (FUNC_TRACE_MASK) ) return True;
    return False ;
}

void printf_func(int func, char *action, long val1, long val2, int val1_unit, int val2_unit)
{
    if( func >= 0 && func < F_FUNCTIONS_NUM )
...
void printf_aswindow(ASWindow *t)
{
  if( t )
  {
...
void printf_asrectangle(ASRectangle *r)
{
  if( r )
  {
...
Status trace_XDestroyWindow (Display * dpy, Window w, const char *filename, int line)
{
  Status ret;
...
#undef XGetGeometry
Status trace_XGetGeometry (Display * dpy,  Drawable d, 
                           Window *root_return, 
			   ...
			   const char *filename, int line)
{
  Status ret;
    ...
    fprintf (stderr, "X<%s(%d):XGetGeometry(0x%lX) returned(0x%lX,%d,%d,%u,%u,%u,%u)  Status=%d\n", filename, line, d, 
             (root_return)?*root_return:0,
	     (x_return)?*x_return:0,(y_return)?*y_return:0,
	     (width_return)?*width_return:0,(height_return)?*height_return:0,
	     (border_width_return)?*border_width_return:0, 
	     (depth_return)?*depth_return:0, ret);
    return ret ;    
}
#endif
...
Bool trace_XCheckMaskEvent( Display* dpy, long mask, XEvent* event, const char* file, int line )
{
  int res ;
...
Bool trace_XCheckTypedEvent( Display* dpy, int event_type, XEvent* event_return, const char* file, int line )
{
  int res ;
...
Bool trace_XCheckTypedWindowEvent( Display* dpy, Window w, int event_type, XEvent* event_return, const char* file, int line )
{
  int res ;
    res = XCheckTypedWindowEvent( dpy, w, event_type, event_return );
    if( res && event_traceble(event_return) )
    {
	fprintf( stderr, "X<%s(%d):XCheckTypedWindowEvent(0x%lX,%d,0x%lX) returned {",file, line, w, event_type, (long)event_return);
	printf_xevent( event_return );
	fprintf( stderr, "} Status=%d\n", res );
    }
    return res ;
...
Bool trace_XCheckWindowEvent( Display* dpy, Window w, long event_mask, XEvent* event_return, const char* file, int line)
{
  int res ;
    res = XCheckWindowEvent( dpy, w, event_mask, event_return);
    if( res && event_traceble(event_return) )
    {
	fprintf( stderr, "X<%s(%d):XCheckWindowEvent(0x%lX,0x%lX,0x%lX) returned {",file, line, w, event_mask, (long)event_return);
	printf_xevent( event_return );
	fprintf( stderr, "} Status=%d\n", res );
    }
    return res ;
...
#undef XNextEvent
int trace_XNextEvent( Display* dpy, XEvent* event, const char* file, int line)
{
  int res ;
...
#undef XPeekEvent
int trace_XPeekEvent(Display* dpy, XEvent* event_return, const char* file, int line )
{
  int res ;
...
#undef XMaskEvent
int trace_XMaskEvent(Display* dpy, long event_mask, XEvent* event_return, const char* file, int line )
{
  int res ;
...
#undef XPutBackEvent
int trace_XPutBackEvent(Display* dpy, XEvent* event, const char* file, int line )
{
  int res ;
...
#undef XWindowEvent
int trace_XWindowEvent(Display* dpy,  Window w, long event_mask, XEvent* event_return, const char* file, int line)
{
  int res ;
    res = XWindowEvent( dpy,  w, event_mask, event_return);
    if( event_traceble(event_return) )
    {
	fprintf( stderr, "X<%s(%d):XWindowEvent(0x%lX,0x%lX,0x%lX) returned {",file, line, w, event_mask, (long)event_return);
	printf_xevent( event_return );
	fprintf( stderr, "} Status=%d\n", res );
    }
    return res ;
...
int trace_UnloadImage (Pixmap pixmap, const char *filename, int line)
{
  int res ;
...
int trace_UnloadMask (Pixmap mask, const char *filename, int line)
{
  int res ;
...
Bool trace_load_font ( const char *font_name, struct MyFont *font, const char *file, int line)
{
  int res ;
    ...
    fprintf( stderr, "L<%s(%d):load_font(\"%s\",0x%lX) returned: %d.\n",file, line, font_name, (unsigned long)font, res );

    return res ;
}
#undef unload_font
extern void unload_font (struct MyFont *font);
void trace_unload_font (struct MyFont *font, const char *file, int line)
{
...
ASWindow *trace_AddWindow (Window w, const char *filename, int line)
{
  ASWindow *asres ;
...
Bool trace_SetFocus (Window w, ASWindow *Fw, const char *filename, int line)
{
  Bool bres ;
...
void trace_SetupFrame (ASWindow *tmp_win, Bool sendEvent, const char *filename, int line)
{
...
unsigned int trace_ResizeFrame (struct ASWindow * t, struct ASRectangle * new_frame, unsigned int todo, const char *filename, int line)
{
  unsigned int uires ;
...
void trace_ResizeClient (struct ASWindow * t, struct ASRectangle * new_client, const char *filename, int line)
{
...
void trace_DispatchEvent (XEvent *event, const char *filename, int line)
{
    if(event_traceble(event))
    {
      ASWindow *tmp_win ;
...
void trace_ReparentIt (ASWindow * t, Window to_win, const char *filename, int line)
{
...
extern void ExecuteFunction (FunctionCode func, char *action, Window in_w,
		 ASWindow * tmp_win, XEvent * eventp, unsigned long context,
		 long val1, long val2, int val1_unit, int val2_unit,
		 MenuRoot * menu, int Module);
void trace_ExecuteFunction (int func, char *action, Window in_w,
		 ASWindow * tmp_win, XEvent * eventp, unsigned long context,
		 long val1, long val2, int val1_unit, int val2_unit,
		 MenuRoot * menu, int Module,
		 const char *filename, int line)
