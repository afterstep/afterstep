...
parse_multichar_part( unsigned char **data )
{
  ...
  unsigned char c;
    ...
    if( (c = parse_singlechar(data,"],")) == 0x00 ) return NULL;
    if( **data == '-' ) 
    {/* range */
	part.size = 3 ;
	(*data)++ ;
...
fix_skip_table( reg_exp* rexp )
{
  ...
  unsigned char c ;
    ...
    for( i = 0 ; i < 256 ; i++ ) rexp->skip[i] = rexp->size ;
...
/* there could be up to 255 possible charcters :) - that excludes 0x00 */
  ...
  unsigned char r_start, r_end ;
	...
	if( *ptr == 0x01 )
	{
	    ...
	    {
		while( r_start < r_end ) scale[r_start++] = 1 ;
...
{/* this is a non-reentrant function in order to conserve memory */
  ...
  reg_exp_sym *p_sym ; 
	...
	{
	    sym_next-- ;
	    while( sym_next > sym_buf && *sym_next ) sym_next-- ;
...
char* place_singlechar( char* trg, unsigned char c )
{
    if( c == '*' || c == '?' || c == '[' || 
        c == ']' || c == '!' || c == ',' || c == '-') 
	*(trg++) = '\\' ;
  ...
  char *trg_start, *trg ;
	...
	for( i = 0 ; i < curr->size ; i++ ) 
	{
		...
		if( *ptr == '*' || *ptr == '?' || *ptr == '[' || 
		    *ptr == ']' || *ptr == '!' || *ptr == ',' || *ptr == '-') 
		    size++ ; /* for prepending \ */
		else if( *ptr == 0x01 ) size+=2 ; /* for comma */
		size++ ;    	
	    }
	    ptr++;
	}
	...
	if(curr->lead_len<0) 
	    *(trg++) = '*' ;
	else for( i = 0 ; i < curr->lead_len ; i++ )
	    *(trg++) = '?' ;
        tail = curr->symbols ;
	...
	for( i = curr->size-1 ; i >= 0 ; i-- ) 
	{
	  ...
	  int part_type ;
	    ...
	    while( ptr > curr->symbols && *ptr ) ptr-- ;
	    tail = ptr ;
	    if( ptr > curr->symbols ) ptr++ ;
...
void make_offsets( wild_reg_exp *wrexp )
{
  reg_exp *curr ;
  short int offset, fixed ;
	...
	for( curr = wrexp->head ; curr ; curr = curr->next )
	{
	    if( curr->lead_len  < 0 ) fixed = 0 ;
	    else offset += curr->lead_len ;
	...
	
	{
	  unsigned char *ptr = curr->symbols ;
	    fprintf( stderr, "\t\t offsets : (%d<%s>,%d<%s>)\n", 
		     curr->left_offset, (curr->left_fixed)?"fixed":"not fixed",
...
*/
	...
	{/* we have a match - now try to match other regexps :*/
	  int bres = 0 ;
	    ...
	    if( bres == 0 ) return 0; /* hooorrraaaaayyyyy !!!!!!!!!! */
	}
	/* ouch, we have a mismatch ! */ 
	skip = pos - offset + 1 ; 
	/* additional heuristic here: */
	if( skip < rexp->skip[c] ) skip = rexp->skip[c] ;
...
match_wild_reg_exp( char* string, wild_reg_exp *wrexp )
{
  size_t len ;
    ...
    len = strlen(string);    
    if( len < wrexp->hard_total+wrexp->soft_total ) return -1 ; /* string is too short to match criteria */
...
compare_wild_reg_exp( wild_reg_exp *wrexp1, wild_reg_exp *wrexp2 )
{
  int strcmp_res, cmp_res ;
    if( wrexp1 == wrexp2 ) return 0;
    if( wrexp1 == NULL ) return -1 ;
    if( wrexp2 == NULL ) return  1 ;
    strcmp_res = strcmp( wrexp1->raw, wrexp2->raw );
    if( strcmp_res == 0 ) return 0 ;
	...
	{   /*  the more wildcrads we have - the less strict pattern it is
	    ...
	    cmp_res = -wrexp1->wildcards_num + wrexp2->wildcards_num ;
    	    if( cmp_res == 0 ) cmp_res = strcmp_res ;
	}
    }
    return cmp_res ;
}
