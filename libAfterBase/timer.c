...
static void
timer_subtract_times (time_t * sec1, time_t * usec1, time_t sec2,
		      time_t usec2)
{
...
Bool 
timer_delay_till_next_alarm (time_t * sec, time_t * usec)
{
  Timer *timer;
  time_t tsec, tusec;
  ...
  for (timer = timer_first; timer != NULL; timer = (*timer).next)
    if ((*timer).sec < tsec
	|| ((*timer).sec == tsec && (*timer).usec <= tusec))
      {
	tsec = (*timer).sec;
	tusec = (*timer).usec;
      }
...
Bool 
timer_remove_by_data (void *data)
{
  Bool success = False;
  Timer *timer;
  for (timer = timer_first; timer != NULL; timer = (*timer).next)
