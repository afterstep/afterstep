...
malloc_tmp (size_t size)
{
  ...
  for (i = 0; i < blocks_used; i++)
    if (short_term_storage[i].first != NULL
	&& short_term_storage[i].max_free >= size)
      break;
  if (i < blocks_used)
    block = &(short_term_storage[i]);
	...
	{			/* use this chunk */
	  if (ptr->size > size + sizeof (allocated_block) * 2)
		...
		block->last = new_ptr;
	      ptr->next = new_ptr;
	      new_ptr->size =
		ptr->size - (((void *) new_ptr) - ((void *) ptr) +
			     sizeof (allocated_block) * 2);
...
free_tmp (void *ptr)
{
  register int i;
  short_term_block *block = short_term_storage;
  allocated_block *owner =
  (allocated_block *) (ptr - sizeof (allocated_block));
  if (ptr == NULL)
    return;

  for (i = 0; i < blocks_used; i++, block++)
    if (block->first != NULL)
      if (ptr > (void *) (block->first)
	  && ptr < (void *) (block->first) + BLOCK_SIZE)
	break;
