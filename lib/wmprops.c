...
void setup_compatibility_props( ScreenInfo *scr )
{
    ...
    set_multi32bit_property (scr->Root, _XA_NET_SUPPORTED, XA_ATOM, 31,
                             _XA_NET_SUPPORTED,
                             _XA_NET_CLIENT_LIST,
...
Bool read_as_background (ASWMProps *wmprops, Bool deleted )
{
    ...
    if( wmprops )
	{
        ...
        res = read_as_property( wmprops->selection_window, _AS_BACKGROUND,
                              ...
                              &(wmprops->as_backgrounds_version),
                              &(wmprops->as_backgrounds_data));
    }
    return res;
}
...
init_wmprop_root_handlers ()
{
    ...
    for( i = 0 ; WMPropsDescriptions_root[i].id_variable != NULL ; i++ )
    {
        add_hash_item ( wmprop_root_handlers,
                        (ASHashableValue) *(WMPropsDescriptions_root[i].id_variable),
...
read_root_wmprops( ASWMProps *wmprops, Bool do_cleanup )
{
    ...
    if( all_props )
    {
        while (props_num-- > 0)
        {
            if (get_hash_item (wmprop_root_handlers, (ASHashableValue) all_props[props_num], (void **)&descr) == ASH_Success)
                if (descr != NULL)
                {
...
read_volitile_wmprops( ASWMProps *wmprops )
{
    ...
    if( all_props )
    {
        while (props_num-- > 0)
        {
            if (get_hash_item (wmprop_volitile_handlers, (ASHashableValue) all_props[props_num], (void **)&descr) == ASH_Success)
                if (descr != NULL)
...
set_desktop_num_prop( ASWMProps *wmprops, long new_desk, Window vroot, Bool add )
{
    if (wmprops)
	{
        ...
        set_32bit_proplist( wmprops->scr->Root, _AS_DESK_NUMBERS, XA_CARDINAL, &(wmprops->as_desk_numbers[0]), wmprops->as_desk_num );
        set_32bit_proplist( wmprops->scr->Root, _XA_NET_VIRTUAL_ROOTS, XA_WINDOW, &(wmprops->virtual_roots[0]), wmprops->desktop_num );
...
set_xrootpmap_id (ASWMProps *wmprops, Pixmap new_pmap )
{
    if (wmprops)
	{
		...
		XFlush( dpy );
		if( wmprops->root_pixmap != None && wmprops->root_pixmap != ParentRelative &&
...
handle_wmprop_event( ASWMProps *wmprops, XEvent *event )
{
	...
	if( event != NULL && wmprops != NULL )
		if( event->type == PropertyNotify )
		{
			...
			{
				if( get_hash_item (wmprop_volitile_handlers, (ASHashableValue) (event->xproperty.atom), (void **)&descr) != ASH_Success)
					descr = NULL ;
			}else if( event->xproperty.window == wmprops->scr->Root )
            	if( get_hash_item (wmprop_root_handlers, (ASHashableValue) (event->xproperty.atom), (void **)&descr) != ASH_Success)
					descr = NULL ;
			if( descr )
			{
