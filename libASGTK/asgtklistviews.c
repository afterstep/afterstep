...
asgtk_simple_list_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_TREE_VIEW, "ASGtkSimpleList", &id_info, 0);
	}
...
static void
asgtk_simple_list_style_set (GtkWidget * widget, GtkStyle * prev_style)
{
...
void
asgtk_simple_list_set_sel_handling (ASGtkSimpleList * self, GObject * owner, _ASGtkList_sel_handler handler)
{
...
void
asgtk_simple_list_purge (ASGtkSimpleList * self)
{
	g_return_if_fail (ASGTK_IS_SIMPLE_LIST (self));
...
void
asgtk_simple_list_append (ASGtkSimpleList * self, const char *name, gpointer data)
{
	GtkTreeIter   iter;

	gtk_list_store_append (GTK_LIST_STORE (self->tree_model), &iter);
	gtk_list_store_set (GTK_LIST_STORE (self->tree_model), &iter, 0, name, 1, data, -1);
	++(self->items_count);
