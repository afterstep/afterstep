...
static void   asgtk_xml_view_tag_selected (GtkTreeSelection * selection, gpointer user_data);
...
asgtk_xml_view_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_VBOX, "ASGtkXMLView", &id_info, 0);
	}
...
asgtk_xml_editor_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_WINDOW, "ASGtkXMLEditor", &id_info, 0);
	}
...
get_validated_text (ASGtkXMLView * xe, Bool selection, Bool report_success)
{
	...
	do
	{
		...
		while (text[char_count] != '\0')
		{
			...
			if (xb.state >= 0)
			{
				if (isspace (text[char_count]) ||
					text[char_count] == '<' ||
					text[char_count] == '>' ||
					text[char_count] == '=' || text[char_count] == '\"' || text[char_count] == '/')
					last_separator = char_count;
			}
			++char_count;
			if ((xb.state == ASXML_Start && xb.tags_count > 0 && xb.level == 0) || xb.state < 0)
				break;
		}
		...
		if (xb.state < 0)
		{
			...
			{
			 case ASXML_BadStart:
				 asgtk_warning2 (GTK_WIDGET (xe), "Text encountered before opening tag bracket - not XML format", NULL,
								 NULL);
				 break;
			 case ASXML_BadTagName:
				 asgtk_warning2 (GTK_WIDGET (xe), "Invalid characters in tag name", NULL, NULL);
				 break;
			 case ASXML_UnexpectedSlash:
				 asgtk_warning2 (GTK_WIDGET (xe), "Unexpected '/' encountered", NULL, NULL);
				 break;
			 case ASXML_UnmatchedClose:
				 asgtk_warning2 (GTK_WIDGET (xe), "Closing tag encountered without opening tag", NULL, NULL);
				 break;
			 case ASXML_BadAttrName:
				 asgtk_warning2 (GTK_WIDGET (xe), "Invalid characters in attribute name", NULL, NULL);
				 break;
			 case ASXML_MissingAttrEq:
				 asgtk_warning2 (GTK_WIDGET (xe), "Attribute name not followed by '=' character", NULL, NULL);
				 break;
			 default:
				 asgtk_warning2 (GTK_WIDGET (xe), "Premature end of the input", NULL, NULL);
				 break;
			}
...
on_refresh_clicked (GtkButton * button, gpointer user_data)
{
	...
	if (xe->entry)
	{
		...
		if (text == NULL)
			return;
		im = compose_asimage_xml (asv, get_screen_image_manager (NULL), get_screen_font_manager (NULL), text,
...
on_save_clicked (GtkButton * button, gpointer user_data)
{
	...
	if (filename)
	{
		if (filename != xe->entry->fullfilename && strcmp (filename, xe->entry->fullfilename) != 0)
		{
			if (CheckFile (filename) == 0)
			{
				if (asgtk_yes_no_question1
					(GTK_WIDGET (xe), "File with the name \"%s\" already exists. Overwrite it ?", filename))
				{
					unlink (filename);
				} else
				{
...
check_save_changes (ASGtkXMLView * xe)
{
	if (xe->dirty && xe->entry)
	{
		xe->dirty = False;
		if (asgtk_yes_no_question1
			(GTK_WIDGET (xe), "Contents of the xml script \"%s\" changed. Save changes ?", xe->entry->name))
...
static char  *ASXMLScriptTags[MAX_ASXML_SCRIPT_TAG][2] = {
	{"text", "\n<text id=\"new_id\" font=\"font\" point=\"size\" fgcolor=\"color\""
	 " bgcolor=\"color\" fgimage=\"image_id\" bgimage=\"image_id\" " " spacing=\"points\">Insert Text here</text>\n"},
	...
	{"gradient", "\n<gradient id=\"new_id\" angle=\"degrees\" refid=\"refid\""
	 " width=\"pixels\" height=\"pixels\" colors =\"color1 [...]\"" " offsets=\"fraction1 [...]\"/>\n"},
...
make_xml_tags_list (ASGtkXMLView * xe)
{
	...
	g_signal_connect (xe->tags_list, "row_activated", G_CALLBACK (asgtk_xml_view_tag_activate), xe);
	...
	for (i = 0; ASXMLScriptTags[i][0]; ++i)
	{
		...
		gtk_list_store_set (store, &iter,
							...
							1, ASXMLScriptTags[i][1], 2, ASXMLScriptTagsHelp[i] ? ASXMLScriptTagsHelp[i] : "", -1);
	}
...
on_insert_color_clicked (GtkWidget * button, gpointer data)
{
	...
	if (xe->color_sel != NULL)
	{
		...
		if (col_str)
		{
			...
			gtk_text_buffer_get_iter_at_mark (buffer, &start, gtk_text_buffer_get_mark (buffer, "insert"));
...
on_color_browser_clicked (GtkWidget * button, gpointer data)
{
	...
	if (xe->color_sel == NULL)
	{
		...
		select_btn =
			asgtk_color_selection_add_main_button (xe->color_sel, GTK_STOCK_ADD, G_CALLBACK (on_insert_color_clicked),
												   xe);
...
//  gtk_container_add (GTK_CONTAINER (xe), main_vbox);
	...
	asgtk_image_view_set_aspect (xe->image_view, get_screen_width (NULL), get_screen_height (NULL));
	...
	xe->color_browser_btn =
		asgtk_add_button_to_box (NULL, GTK_STOCK_ADD, "Color Selector", G_CALLBACK (on_color_browser_clicked), xe);
	gtk_box_pack_end (GTK_BOX (tags_vbox), xe->color_browser_btn, FALSE, FALSE, 0);

	insert_tag_btn =
		asgtk_add_button_to_box (NULL, GTK_STOCK_ADD, "Insert Tag template", G_CALLBACK (on_insert_tag_clicked), xe);
	...
	xe->render_selection_btn =
		asgtk_add_button_to_box (NULL, GTK_STOCK_SAVE_AS, "Render Selection", G_CALLBACK (on_refresh_clicked), xe);
	xe->refresh_btn = asgtk_add_button_to_box (NULL, GTK_STOCK_REFRESH, NULL, G_CALLBACK (on_refresh_clicked), xe);
	xe->validate_btn =
		asgtk_add_button_to_box (NULL, GTK_STOCK_REFRESH, "Validate XML", G_CALLBACK (on_validate_clicked), xe);
	xe->save_btn =
		asgtk_add_button_to_box (GTK_BOX (save_hbox), GTK_STOCK_SAVE, NULL, G_CALLBACK (on_save_clicked), xe);
	xe->save_as_btn =
		asgtk_add_button_to_box (GTK_BOX (save_hbox), GTK_STOCK_SAVE_AS, NULL, G_CALLBACK (on_save_clicked), xe);
...
void
asgtk_xml_view_set_entry (ASGtkXMLView * xe, struct ASImageListEntry *entry)
{
...
void
asgtk_xml_view_file_change_handler (ASGtkXMLView * xe, _ASGtkXMLView_handler change_handler, gpointer user_data)
{
	g_return_if_fail (ASGTK_IS_XML_VIEW (xe));
...
void
asgtk_xml_editor_set_entry (ASGtkXMLEditor * xe, struct ASImageListEntry *entry)
{
...
void
asgtk_xml_editor_file_change_handler (ASGtkXMLEditor * xe, _ASGtkXMLView_handler change_handler, gpointer user_data)
{
	g_return_if_fail (ASGTK_IS_XML_EDITOR (xe));
