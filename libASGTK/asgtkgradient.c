...
static void asgtk_gradient_style_set (GtkWidget * widget,
																			GtkStyle * prev_style);
...
GType asgtk_gradient_get_type (void)
{
	...
	if (!ib_type) {
		...
		ib_type =
				g_type_register_static (GTK_TYPE_DIALOG, "ASGtkGradient", &ib_info,
																0);
	}
...
ASGradientPoint *create_asgrad_point ()
{
...
void destroy_asgrad_point (void *data)
{
...
int compare_asgrad_point (void *data1, void *data2)
{
	...
	if (point2 == NULL)
		return 1;
	if (point1->offset + 0.0001 > point2->offset
			&& point1->offset - 0.0001 < point2->offset)
		return 0;
	return point1->offset > point2->offset ? 1 : -1;
}

static void asgtk_gradient_init (ASGtkGradient * ge)
{
...
static void
asgtk_gradient_color_select (GtkTreeSelection * selection,
														 gpointer user_data)
{
	...
	if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		if ((ge->current_point = (ASGradientPoint *) p) != NULL) {
			gtk_entry_set_text (GTK_ENTRY (ge->color_entry),
													ge->current_point->color_str);
			update_color_preview (ge, ge->current_point->color_str);
			gtk_spin_button_set_value (GTK_SPIN_BUTTON (ge->offset_entry),
																 ge->current_point->offset);
		}
	}
}

static GtkWidget *asgtk_gradient_create_color_list (ASGtkGradient * ge)
{
	GtkWidget *scrolled_win;
	GtkTreeViewColumn *column;

	ge->point_list_model =
			gtk_list_store_new (3, G_TYPE_STRING, G_TYPE_FLOAT, G_TYPE_POINTER);
	ge->point_list =
			gtk_tree_view_new_with_model (GTK_TREE_MODEL (ge->point_list_model));

	column =
			gtk_tree_view_column_new_with_attributes ("Point Offset : ",
																								gtk_cell_renderer_text_new
																								(), "text", 1, NULL);
	gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
	gtk_tree_view_append_column (GTK_TREE_VIEW (ge->point_list), column);
	column =
			gtk_tree_view_column_new_with_attributes ("Point Color : ",
																								gtk_cell_renderer_text_new
																								(), "text", 0, NULL);
	gtk_tree_view_column_set_sizing (column, GTK_TREE_VIEW_COLUMN_AUTOSIZE);
	gtk_tree_view_append_column (GTK_TREE_VIEW (ge->point_list), column);

	gtk_widget_set_size_request (ge->point_list, COLOR_LIST_WIDTH,
															 COLOR_LIST_HEIGHT);
	g_signal_connect (gtk_tree_view_get_selection
										(GTK_TREE_VIEW (ge->point_list)), "changed",
...
static Bool add_point_to_color_list (void *data, void *aux_data)
{
	ASGradientPoint *point = (ASGradientPoint *) data;
	...
	gtk_list_store_set (ge->point_list_model, &iter, 0, point->color_str, 1,
											point->offset, 2, point, -1);

	if (point == ge->current_point)
		gtk_tree_selection_select_iter (gtk_tree_view_get_selection
																		(GTK_TREE_VIEW (ge->point_list)),
																		&iter);
...
static void asgtk_gradient_update_color_list (ASGtkGradient * ge)
{
	gtk_list_store_clear (ge->point_list_model);
	iterate_asbidirlist (ge->points, add_point_to_color_list, ge, NULL,
											 False);
}
...
static void update_color_preview (ASGtkGradient * ge, const char *color)
{
	...
	pb = solid_color2GdkPixbuf (argb, ge->color_preview_width,
															ge->color_preview_height);
	if (pb) {
...
void on_color_clicked (GtkWidget * btn, gpointer data)
{
	...
	int response = 0;
	const char *orig_color =
			gtk_entry_get_text (GTK_ENTRY (ge->color_entry));

	gtk_dialog_add_buttons (cs, GTK_STOCK_CANCEL, GTK_RESPONSE_REJECT,
													GTK_STOCK_OK, GTK_RESPONSE_ACCEPT, NULL);

	if (orig_color && orig_color[0] != '\0')
		asgtk_color_selection_set_color_by_name (ASGTK_COLOR_SELECTION (cs),
																						 orig_color);
	else
		asgtk_color_selection_set_color_by_name (ASGTK_COLOR_SELECTION (cs),
																						 DEFAULT_COLOR_STR);
	...
	if (response == GTK_RESPONSE_ACCEPT) {
		char *color =
				asgtk_color_selection_get_color_str (ASGTK_COLOR_SELECTION (cs));
...
static Bool add_point_to_gradient (void *data, void *aux_data)
{
	ASGradientPoint *point = (ASGradientPoint *) data;
...
static void refresh_gradient_preview (ASGtkGradient * ge)
{
	...
	gradient.offset = offset =
			safemalloc ((ge->points->count + 2) * sizeof (double));
	gradient.color = color =
			safemalloc ((ge->points->count + 2) * sizeof (ARGB32));

	++gradient.offset;
	++gradient.color;
	iterate_asbidirlist (ge->points, add_point_to_gradient, &gradient, NULL,
											 False);
	...
	entry->preview =
			make_gradient (get_screen_visual (NULL), &gradient, width, height,
										 SCL_DO_ALL, ASA_ASImage, 0, ASIMAGE_QUALITY_DEFAULT);
...
static void on_direction_clicked (GtkWidget * widget, gpointer data)
{
	...
	if (gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ge->tl2br_radio)))
		new_type = GRADIENT_TopLeft2BottomRight;
					 ...
					 (GTK_TOGGLE_BUTTON (ge->t2b_radio)))
		new_type = GRADIENT_Top2Bottom;
	else if (gtk_toggle_button_get_active
					 (GTK_TOGGLE_BUTTON (ge->bl2tr_radio)))
...
static void on_size_clicked (GtkWidget * widget, gpointer data)
{
	ASGtkGradient *ge = ASGTK_GRADIENT (data);
...
static void on_delete_point_clicked (GtkWidget * widget, gpointer data)
{
	ASGtkGradient *ge = ASGTK_GRADIENT (data);
...
query_point_values (ASGtkGradient * ge, ASGradientPoint * point)
{
	...
	point->offset =
			gtk_spin_button_get_value (GTK_SPIN_BUTTON (ge->offset_entry));
	...
	if (color == NULL || color[0] == '\0') {
		...
		if (parse_argb_color (color, &(point->color_argb)) == color)
			asgtk_warning2 (GTK_WIDGET (ge),
											"Color \"%s\" cannot be translated. It is probably a mistyped color name.",
											color, NULL);
		point->color_str = mystrdup (color);
	}
}

static void on_add_point_clicked (GtkWidget * widget, gpointer data)
{
	ASGtkGradient *ge = ASGTK_GRADIENT (data);
...
static void on_apply_point_clicked (GtkWidget * widget, gpointer data)
{
	ASGtkGradient *ge = ASGTK_GRADIENT (data);
...
static void
color_preview_size_alloc (GtkWidget * widget, GtkAllocation * allocation,
													gpointer user_data)
{
	ASGtkGradient *ge = ASGTK_GRADIENT (user_data);
...
GtkWidget *asgtk_gradient_new ()
{
	...
	gtk_table_attach_defaults (GTK_TABLE (table), ge->tl2br_radio, 1, 2, 0,
														 1);
	ge->bl2tr_radio =
			gtk_radio_button_new_with_label_from_widget (GTK_RADIO_BUTTON
																									 (ge->tl2br_radio),
																									 "Bottom-Left to Top-Right");
	gtk_table_attach_defaults (GTK_TABLE (table), ge->bl2tr_radio, 1, 2, 1,
														 2);
	gtk_widget_show_all (table);
	gtk_widget_show (table);
	colorize_gtk_widget (frame, get_colorschemed_style_normal ());

	g_signal_connect ((gpointer) ge->l2r_radio, "clicked",
										G_CALLBACK (on_direction_clicked), ge);
	g_signal_connect ((gpointer) ge->tl2br_radio, "clicked",
										G_CALLBACK (on_direction_clicked), ge);
	g_signal_connect ((gpointer) ge->t2b_radio, "clicked",
										G_CALLBACK (on_direction_clicked), ge);
	g_signal_connect ((gpointer) ge->bl2tr_radio, "clicked",
										G_CALLBACK (on_direction_clicked), ge);


	ge->screen_width_check =
			gtk_check_button_new_with_label ("Use screen width");
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (ge->screen_width_check),
																TRUE);
	ge->width_entry = gtk_spin_button_new_with_range (1, 10000, 1);
	gtk_widget_set_sensitive (ge->width_entry, FALSE);
	ge->screen_height_check =
			gtk_check_button_new_with_label ("Use screen height");
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON
																(ge->screen_height_check), TRUE);
	ge->height_entry = gtk_spin_button_new_with_range (1, 10000, 1);
	gtk_widget_set_sensitive (ge->height_entry, FALSE);

	g_signal_connect ((gpointer) ge->screen_width_check, "clicked",
										G_CALLBACK (on_size_clicked), ge);
	g_signal_connect ((gpointer) ge->screen_height_check, "clicked",
										G_CALLBACK (on_size_clicked), ge);


	ge->size_frame = gtk_frame_new ("Rendered gradient size : ");
	gtk_box_pack_start (GTK_BOX (list_vbox), ge->size_frame, FALSE, FALSE,
											5);
	...
	gtk_table_attach_defaults (GTK_TABLE (table), gtk_label_new ("Width : "),
														 0, 1, 0, 1);
	gtk_table_attach_defaults (GTK_TABLE (table), ge->width_entry, 1, 2, 0,
														 1);
	gtk_table_attach_defaults (GTK_TABLE (table),
														 gtk_label_new ("Height : "), 2, 3, 0, 1);
	gtk_table_attach_defaults (GTK_TABLE (table), ge->height_entry, 3, 4, 0,
														 1);
	gtk_table_attach (GTK_TABLE (table), ge->screen_width_check, 0, 2, 1, 2,
										GTK_FILL, GTK_FILL, 10, 0);
	gtk_table_attach (GTK_TABLE (table), ge->screen_height_check, 2, 4, 1, 2,
										GTK_FILL, GTK_FILL, 10, 0);
	...
	gtk_box_pack_start (GTK_BOX (list_vbox), scrolled_window, FALSE, FALSE,
											0);
	...
	gtk_table_attach (GTK_TABLE (table), ge->color_entry, 1, 2, 0, 1,
										GTK_SHRINK, GTK_SHRINK, 2, 2);
	gtk_table_attach (GTK_TABLE (table), ge->offset_entry, 3, 4, 0, 1,
										GTK_SHRINK, GTK_SHRINK, 2, 2);

	frame = gtk_frame_new (NULL);
	colorize_gtk_tree_view_window (GTK_WIDGET (frame));
	gtk_widget_set_size_request (frame, COLOR_PREVIEW_WIDTH,
	...
	gtk_table_attach (GTK_TABLE (table), btn, 0, 2, 1, 2, GTK_FILL,
										GTK_SHRINK, 2, 2);
	g_signal_connect ((gpointer) btn, "clicked",
										G_CALLBACK (on_color_clicked), ge);
	...
	g_signal_connect ((gpointer) btn, "clicked",
										G_CALLBACK (on_delete_point_clicked), ge);

	btn = gtk_button_new_from_stock (GTK_STOCK_ADD);
	colorize_gtk_widget (GTK_WIDGET (btn), get_colorschemed_style_button ());
	gtk_box_pack_end (GTK_BOX (hbox), btn, FALSE, FALSE, 0);
	g_signal_connect ((gpointer) btn, "clicked",
										G_CALLBACK (on_add_point_clicked), ge);
	...
	gtk_box_pack_end (GTK_BOX (hbox), btn, FALSE, FALSE, 0);
	g_signal_connect ((gpointer) btn, "clicked",
										G_CALLBACK (on_apply_point_clicked), ge);
	...
	gtk_widget_set_size_request (GTK_WIDGET (ge->image_view), PREVIEW_WIDTH,
															 PREVIEW_HEIGHT);
	gtk_box_pack_end (GTK_BOX (main_hbox), GTK_WIDGET (ge->image_view), TRUE,
										TRUE, 0);
	asgtk_image_view_set_aspect (ge->image_view, get_screen_width (NULL),
	...
	g_signal_connect (G_OBJECT (btn), "toggled",
										G_CALLBACK (asgtk_image_view_screen_aspect_toggle),
										(gpointer) ge->image_view);
	...
	colorize_gtk_widget (btn, get_colorschemed_style_normal ());
	g_signal_connect (G_OBJECT (btn), "toggled",
										G_CALLBACK (asgtk_image_view_scale_to_view_toggle),
										(gpointer) ge->image_view);
	gtk_button_set_alignment (GTK_BUTTON (btn), 1.0, 0.5);
	asgtk_image_view_add_detail (ge->image_view, btn, 0);

	btn =
			asgtk_add_button_to_box (NULL, GTK_STOCK_REFRESH, NULL,
															 G_CALLBACK (on_refresh_clicked), ge);
...
static Bool collect_gradient_offsets_str (void *data, void *aux_data)
{
	ASGradientPoint *point = (ASGradientPoint *) data;
...
static Bool collect_gradient_colors_str (void *data, void *aux_data)
{
	ASGradientPoint *point = (ASGradientPoint *) data;
	char **pstr = (char **)aux_data;
...
char *asgtk_gradient_get_xml (ASGtkGradient * ge, char **mini)
{
	...
	if (ge->points->count > 0) {
		...
		iterate_asbidirlist (ge->points, collect_gradient_offsets_str,
												 &offsets, NULL, False);
		iterate_asbidirlist (ge->points, collect_gradient_colors_str, &colors,
												 NULL, False);
		...
		xml =
				safemalloc (64 + 10 + 10 + strlen (colors) + 4 + 4 +
										strlen (offsets) + 32 + 32 + 2 + 1);
		...
		if (mini) {
			*mini = safemalloc (pos + 64);
			sprintf (*mini,
							 "%s\" width=\"$minipixmap.width\" height=\"$minipixmap.height\"/>",
							 xml);
		}
