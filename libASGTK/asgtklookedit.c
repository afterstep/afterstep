...
asgtk_mystyle_edit_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_VBOX, "ASGtkMyStyleEdit", &id_info, 0);
	}
...
static void
fill_text_style_combo_box (GtkWidget * w)
{
	int           i = 0;
...
void
set_container_child_sensitive (GtkWidget * child, gpointer data)
{
...
void
set_container_child_insensitive (GtkWidget * child, gpointer data)
{
	GtkWidget    *to_skip = data ? GTK_WIDGET (data) : NULL;
...
asgtk_mystyle_edit_set_line_enabled (ASGtkMyStyleEdit * self, ASGtkMyStyleOptions opt, Bool enabled)
{
	...
	if (toggle)
	{
		...
		gtk_button_set_image (GTK_BUTTON (toggle),
							  gtk_image_new_from_stock (enabled ? GTK_STOCK_YES : GTK_STOCK_NO, GTK_ICON_SIZE_BUTTON));
	...
	if (container)
		gtk_container_forall (container, enabled ? set_container_child_sensitive : set_container_child_insensitive,
							  toggle);
}
...
asgtk_mystyle_edit_set_background_type (ASGtkMyStyleEdit * self, int type)
{
	...
	} else if (type >= TEXTURE_TEXTURED_START && type <= TEXTURE_TEXTURED_END)
	{
		...
		if (type == TEXTURE_SHAPED_SCALED_PIXMAP)
			back_type = get_flags (self->flags, ASGTK_MYSTYLE_SLICED) ? ASGtkMSB_SlicedShaped : ASGtkMSB_ScaledShaped;
...
GtkWidget    *create_mystyle_editor_interface (GtkWidget * mystyle_editor /* assumed descendand from vbox */ );
...
asgtk_look_edit_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_VBOX, "ASGtkLookEdit", &id_info, 0);
	}
...
build_mystyles_panel (ASGtkMyStylesPanel * panel)
{
	...
	asgtk_mystyle_edit_set_mystyles_list (ASGTK_MYSTYLE_EDIT (panel->mystyle_editor),
										  ASGTK_SIMPLE_LIST (panel->list)->tree_model);
	asgtk_mystyle_edit_set_name_visible (ASGTK_MYSTYLE_EDIT (panel->mystyle_editor), False);

	asgtk_simple_list_set_sel_handling (ASGTK_SIMPLE_LIST (panel->list), G_OBJECT (panel->mystyle_editor),
	...
	panel->list_add_btn =
		asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_ADD, "Add", G_CALLBACK (on_add_mystyle_btn_clicked), panel);
	panel->list_del_btn =
		asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_DELETE, "Delete", G_CALLBACK (on_add_mystyle_btn_clicked),
								 panel);
	panel->list_rename_btn =
		asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_PREFERENCES, "Rename",
								 G_CALLBACK (on_add_mystyle_btn_clicked), panel);
	ASGTK_PACK_END;
	ASGTK_PACK_BEGIN (panel->list_hbtn_box2);
	panel->list_tolib_btn =
		asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_CONVERT, "Add to Library",
								 G_CALLBACK (on_add_to_library_mystyle_btn_clicked), panel);
	panel->list_saveas_btn =
		asgtk_add_button_to_box (ASGTK_PACK_BOX, GTK_STOCK_SAVE_AS, "Save As",
								 G_CALLBACK (on_save_as_mystyle_btn_clicked), panel);
	ASGTK_PACK_END;
	InitSession ();
