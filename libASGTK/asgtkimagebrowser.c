...
asgtk_image_browser_get_type (void)
{
	...
	if (!ib_type)
	{
		...
		ib_type = g_type_register_static (GTK_TYPE_WINDOW, "ASGtkImageBrowser", &ib_info, 0);
	}
...
static char  *
asgtk_image_browser_make_fullfilename (ASGtkImageBrowser * ib, const char *filename)
{
...
asgtk_image_browser_scale_toggle (GtkWidget * checkbutton, gpointer data)
{
	...
	asgtk_image_view_set_resize (ib->preview,
								 ...
								 ASGTK_IMAGE_VIEW_SCALE_TO_VIEW : 0, ASGTK_IMAGE_VIEW_SCALE_TO_VIEW);
}
...
asgtk_image_browser_create_dir_list (ASGtkImageBrowser * ib, GtkWidget * list_vbox)
{
	...
	column = gtk_tree_view_column_new_with_attributes ("Folders", gtk_cell_renderer_text_new (), "text", 0, NULL);
	...
	g_signal_connect (ib->dir_list, "row_activated", G_CALLBACK (asgtk_image_browser_dir_activate), ib);
...
asgtk_image_browser_change_dir (ASGtkImageBrowser * ib, const char *dir)
{
	...
	LOCAL_DEBUG_OUT (" changing dir for  %p: current dir = \"%s\", new dir = \"%s\"", ib,
