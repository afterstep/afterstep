...
asgtk_image_dir_get_type (void)
{
	...
	if (!id_type)
	{
		...
		id_type = g_type_register_static (GTK_TYPE_SCROLLED_WINDOW, "ASGtkImageDir", &id_info, 0);
	}
...
{
	gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (id), GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
...
void
asgtk_image_dir_refresh (ASGtkImageDir * id)
{
	int           items = 0;
	char         *curr_sel;
	...
	if (id->fulldirname)
	{
		...
		id->entries = get_asimage_list (get_screen_visual (NULL), id->fulldirname,
										0, get_screen_image_manager (NULL)->gamma, 0, 0, 0, &count, NULL);
		...
		while (curr)
		{
			...
			if (mini_ext_len > 1)
			{
				...
				else
				{
					...
					if (name_len > mini_ext_len)
						mini =
							(strncmp (&(curr->name[name_len - mini_ext_len]), id->mini_extension, mini_ext_len) == 0);
				}
			}
			if ((!mini && curr->type <= ASIT_Supported) || get_flags (id->flags, ASGTK_ImageDir_ListAll))
			{
				...
				sprintf (date_str, "%02d-%02d-%02d %02d:%02d",
						 (mt->tm_year + 1900) % 100, mt->tm_mon + 1, mt->tm_mday, mt->tm_hour, mt->tm_min);
				...
				gtk_list_store_set (GTK_LIST_STORE (id->tree_model), &iter,
									...
									ASGTK_ImageDir_Col_Date_No, date_str, ASGTK_ImageDir_Cols, curr, -1);
				if (++items == 1)
...
make_mini_for_image_entry (ASGtkImageDir * id, ASImageListEntry * entry, const char *mini_fullfilename)
{
	...
	if (entry->type == ASIT_XMLScript)
	{
		...
		sprintf (xml, "<scale width=\"$minipixmap.width\" height=\"$minipixmap.height\"><img src=\"%s\"/></scale>",
				 entry->name);
	...
	{
		ASImage      *thumbnail =
			scale_asimage (get_screen_visual (NULL), entry->preview, 24, 24, ASA_ASImage, 0, ASIMAGE_QUALITY_DEFAULT);
		if (thumbnail)
		{
