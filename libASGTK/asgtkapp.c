...
get_colorschemed_style_button ()
{
	if (ASGtkStyleButton == NULL)
	{
		...
		color_name2GdkColor ("HighInactiveText", &(ASGtkStyleButton->fg[GTK_STATE_INSENSITIVE]));
...
void
asgtk_info2 (GtkWidget * main_window, const char *format, const char *detail1, const char *detail2)
{
	GtkWidget    *dialog = gtk_message_dialog_new (GTK_WINDOW (main_window),
												   GTK_DIALOG_DESTROY_WITH_PARENT,
												   GTK_MESSAGE_INFO,
...
void
find_combobox_entry (GtkWidget * widget, gpointer data)
{
