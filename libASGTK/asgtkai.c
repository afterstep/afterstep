...
ASImage2GdkPixbuf (ASImage * im)
{
	...
	if (im)
	{
		...
		if ((imdec =
			 start_image_decoding (get_screen_visual (NULL), im, SCL_DO_ALL, 0, 0, im->width, im->height,
								   NULL)) != NULL)
		{
		...
		pb = gdk_pixbuf_new_from_data (data, GDK_COLORSPACE_RGB, True, 8, im->width, im->height, im->width * 4,
									   free_buffer, NULL);
		if (pb == NULL)
			free (data);
	}
...
GdkPixbuf2ASImage (GdkPixbuf * pixbuf)
{
	...
	if (pixbuf)
	{
		...
		if (gdk_pixbuf_get_colorspace (pixbuf) != GDK_COLORSPACE_RGB
			...
			|| width <= 0 || height <= 0 || rowstride <= 0 || n_channels != 3 + has_alpha)
			return NULL;
		...
		if ((pixels = gdk_pixbuf_get_pixels (pixbuf)) != NULL)
		{
			...
			for (y = 0; y < height; ++y)
			{
				raw2scanline (pixels + y * rowstride, &buf, NULL, width,
							  gdk_pixbuf_get_colorspace (pixbuf) != GDK_COLORSPACE_RGB, has_alpha);
