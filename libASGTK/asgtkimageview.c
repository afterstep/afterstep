...
asgtk_image_view_get_type (void)
{
	...
	if (!iv_type)
	{
		...
		iv_type = g_type_register_static (GTK_TYPE_FRAME, "ASGtkImageView", &iv_info, 0);
	}
...
asgtk_image_view_style_set (GtkWidget * widget, GtkStyle * prev_style)
{
	...
	gtk_widget_modify_bg (iv->view, GTK_STATE_NORMAL, &widget->style->base[GTK_STATE_NORMAL]);
	gtk_widget_modify_bg (iv->view, GTK_STATE_INSENSITIVE, &widget->style->base[GTK_STATE_NORMAL]);
}
...
display_image_view (ASGtkImageView * iv)
{
	...
	{
		if (get_flags (iv->flags, ASGTK_IMAGE_VIEW_TILE_TO_ASPECT) && iv->aspect_x > 0 && iv->aspect_y > 0)
		{
	...
	} else if (get_flags (iv->flags, ASGTK_IMAGE_VIEW_SCALE_TO_ASPECT) && iv->aspect_x > 0 && iv->aspect_y > 0)
	{
	...
	LOCAL_DEBUG_OUT ("####!!! recquisition is %dx%d",
					 GTK_WIDGET (iv->view)->requisition.width, GTK_WIDGET (iv->view)->requisition.height);
#endif
}
...
static void
asgtk_imview_view_size_alloc (GtkWidget * widget, GtkAllocation * allocation, gpointer user_data)
{
	...
	LOCAL_DEBUG_OUT ("####!!! SizeAlloc for %p is %dx%d%+d%+d", widget, allocation->width, allocation->height,
...
set_aspect_ratio_from_image (ASGtkImageView * iv)
{
	...
	if (iv->image_entry && iv->image_entry->preview)
		new_ratio = (gfloat) iv->image_entry->preview->width / (gfloat) iv->image_entry->preview->height;
	else
		new_ratio = 1.0;
...
asgtk_image_view_make_parts (ASGtkImageView * iv, Bool horizontal)
{
	...
	g_signal_connect ((gpointer) iv->scrolled_window, "size-allocate", G_CALLBACK (asgtk_imview_view_size_alloc), iv);
...
void
asgtk_image_view_set_entry (ASGtkImageView * iv, ASImageListEntry * image_entry)
{
...
void
asgtk_image_view_set_aspect (ASGtkImageView * iv, int aspect_x, int aspect_y)
{
	Bool          changed = False;
...
asgtk_image_view_set_resize (ASGtkImageView * iv, unsigned long resize_flags, unsigned long set_mask)
{
	...
	{
		gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (iv->scrolled_window), GTK_POLICY_NEVER, GTK_POLICY_NEVER);
	} else
	{
		gtk_scrolled_window_set_policy (GTK_SCROLLED_WINDOW (iv->scrolled_window),
										GTK_POLICY_AUTOMATIC, GTK_POLICY_AUTOMATIC);
...
asgtk_image_view_refresh (ASGtkImageView * iv, Bool reload_file)
{
	...
	if (iv->image_entry)
	{
		...
		if (reload_file && iv->image_entry->type <= ASIT_Supported)
		{
			...
			LOCAL_DEBUG_OUT ("imman = %p, fullname = \"%s\"", get_screen_image_manager (NULL),
							 iv->image_entry->fullfilename);
			iv->image_entry->preview =
				get_asimage (get_screen_image_manager (NULL), iv->image_entry->fullfilename, 0xffffffff, 100);
...
asgtk_image_view_screen_aspect_toggle (GtkWidget * checkbutton, gpointer data)
{
	...
	if (GTK_TOGGLE_BUTTON (checkbutton)->active)
	{
		...
		asgtk_image_view_set_resize (iv, ASGTK_IMAGE_VIEW_TILE_TO_ASPECT, ASGTK_IMAGE_VIEW_TILE_TO_ASPECT);
	} else
	{
...
asgtk_image_view_scale_to_view_toggle (GtkWidget * checkbutton, gpointer data)
{
	...
	if (GTK_TOGGLE_BUTTON (checkbutton)->active)
		asgtk_image_view_set_resize (iv, ASGTK_IMAGE_VIEW_SCALE_TO_VIEW, ASGTK_IMAGE_VIEW_SCALE_TO_VIEW);
	else
