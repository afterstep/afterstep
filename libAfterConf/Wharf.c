...
TermDef WhevTerms[] = {
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "push", 4, TT_FILENAME,
	 WHEV_PUSH_ID, NULL}
	,
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "close_folder", 12,
	 TT_FILENAME, WHEV_CLOSE_FOLDER_ID, NULL}
	,
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "open_folder", 11,
	 TT_FILENAME, WHEV_OPEN_FOLDER_ID, NULL}
	,
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "close_main", 10,
	 TT_FILENAME, WHEV_CLOSE_MAIN_ID, NULL}
	,
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "open_main", 9,
	 TT_FILENAME, WHEV_OPEN_MAIN_ID, NULL}
	,
	{TF_NO_MYNAME_PREPENDING | TF_SYNTAX_TERMINATOR, "drop", 4, TT_FILENAME,
	 WHEV_DROP_ID, NULL}
	,
	{0, NULL, 0, 0, 0, NULL}
};
...
#define WHARF_FOLDER_TERMS \
	{TF_SPECIAL_PROCESSING|TF_NAMED|TF_USE_TOKENS(1), "", 0, TT_ICON_LIST, WHARF_Wharf_ID, &DummyFuncSyntax}, \
	{TF_SYNTAX_TERMINATOR, WHARF_FOLDER_END, 7, TT_FLAG, WHARF_FolderEnd_ID, NULL}
...
int print_wharf_folder (WharfButton * folder, int level)
{
	int count = 1;
	int my_level = level;
	...
	while (folder) {
		...
		show_progress ("WHARF.FOLDER[%d].BUTTON[%d].set_flags=0x%lX;",
									 my_level, count, folder->set_flags);
		show_progress ("WHARF.FOLDER[%d].BUTTON[%d].title=\"%s\";", my_level,
									 count, folder->title);
		show_progress ("WHARF.FOLDER[%d].BUTTON[%d].width=%d;", my_level,
									 count, folder->width);
		show_progress ("WHARF.FOLDER[%d].BUTTON[%d].height=%d;", my_level,
									 count, folder->height);
		if (folder->contents) {
			int k;
			...
			for (k = 0; k < folder->contents_num; ++k) {
				...
				if (icon)
					while (icon[i] != NULL) {
						show_progress
								("WHARF.FOLDER[%d].BUTTON[%d].CONTNTS[%d].icon[%d]=\"%s\";",
								 my_level, count, k, i, icon[i]);
						++i;
					}
				if (function)
...
void PrintWharfConfig (WharfConfig * config)
{
	...
	if (get_flags (config->set_flags, WHARF_GEOMETRY))
		show_progress ("WHARF.geometry=(0x%lx,%dx%d%+d%+d);",
									 config->geometry.flags, config->geometry.width,
	...
	if (get_flags (config->set_flags, WHARF_FORCE_SIZE))
		show_progress ("WHARF.force_size=(0x%lx,%dx%d%+d%+d);",
									 config->force_size.flags, config->force_size.width,
									 config->force_size.height, config->force_size.x,
									 config->force_size.y);
	...
	if (get_flags (config->set_flags, WHARF_ANIMATE_STEPS_MAIN))
		show_progress ("WHARF.animate_steps_main=%d;",
...
void ParseWharfFolder (FreeStorageElem ** storage_tail,
											 WharfButton ** tail);
...
FreeStorageElem **WharfFolder2FreeStorage (SyntaxDef * syntax,
																					 FreeStorageElem ** tail,
																					 WharfButton * folder, int root);

FreeStorageElem **WharfButton2FreeStorage (SyntaxDef * syntax,
																					 FreeStorageElem ** tail,
																					 WharfButton * button)
{
	...
	if ((new_elem =
		...
		if (len > 0) {
			...
			if (button->folder) {
				...
				WharfFolder2FreeStorage (syntax, &(new_elem->sub->sub),
																 button->folder, False);
