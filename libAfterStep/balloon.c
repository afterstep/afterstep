...
setup_active_balloon_tiles (ASBalloonState * state)
{
	if (state->active->type == ASBalloon_Text)
		add_astbar_label (state->active_bar, 0, 0, 0, ALIGN_CENTER, 1, 1, state->active->data.text.text,
						  state->active->data.text.encoding);
	else
	{
		LOCAL_DEBUG_OUT ("balloon is image: filename = \"%s\", image = %p", state->active->data.image.filename,
						 state->active->data.image.image);
		if (state->active->data.image.image == NULL)
		{
			ASImage      *im =
				get_asimage (ASDefaultScr->image_manager, state->active->data.image.filename, ASFLAGS_EVERYTHING, 100);
			if (im)
			{
				...
				{
					ASImage      *tmp =
						scale_asimage (ASDefaultVisual, im, scale_width, scale_height, ASA_ASImage, 100,
					...
					{
						tmp =
							tile_asimage (ASDefaultVisual, im, 0, 0, tile_width, scale_height, TINT_NONE, ASA_ASImage,
...
display_active_balloon (ASBalloonState * state)
{
	if (state == NULL)
		state = &DefaultBalloonState;
	LOCAL_DEBUG_CALLER_OUT ("active(%p)->window(%lX)->canvas(%p)", state->active, state->active_window,
							state->active_canvas);
	if (state->active != NULL && state->active_window != None && state->active_canvas == NULL)
	{
...
int
destroy_balloon_look (ASBalloonLook * blook)
{
	if (blook == NULL)
		return 0;
...
ASBalloonLook *
ref_balloon_look (ASBalloonLook * blook)
{
	if (blook)
		if (blook->ref_count > 0)
			blook->ref_count++;
...
set_balloon_state_look (ASBalloonState * state, ASBalloonLook * blook)
{
	...
	if (state->look->show && state->active_window == None)
	{
		...
		state->active_window =
			create_visual_window (ASDefaultVisual, ASDefaultRoot, -10, -10, 1, 1, 0, InputOutput,
...
is_balloon_click (XEvent * xe)
{
	...
	for (; state != NULL; state = state->next)
		if (state->active_window != None)
		{
			LOCAL_DEBUG_OUT ("Balloon window is %lX, xbutton.window = %lX, subwindow = %lX", state->active_window,
							 xe->xbutton.window, xe->xbutton.subwindow);
			if (xe->xbutton.window == state->active_window || xe->xbutton.subwindow == state->active_window)
				break;
		}
	return state;
}
...
static void
destroy_asballoon_data (ASBalloon * balloon)
{
...
inline static void
set_asballoon_data_text (ASBalloon * balloon, const char *text, unsigned long encoding)
{
...
ASBalloon    *
create_asballoon_with_text_for_state (ASBalloonState * state, ASTBarData * owner, const char *text,
