...
CARD8 make_component_hilite (int cmp)
{
	if (cmp < 51)
		cmp = 51;
	cmp = (cmp * 12) / 10;
 ...
 ***************************************************************************/
ASImage *grab_root_asimage (ScreenInfo * scr, Window target,
														Bool screenshot)
{
	...
	if (target != None && target != scr->Root) {
		...
		if (XGetGeometry (dpy, target, &wdumm, &tx, &ty, &tw, &th, &bw, &udumm)
				!= 0) {
			/* we need root window position : */
			XTranslateCoordinates (dpy, target, scr->Root, 0, 0, &tx, &ty,
														 &wdumm);
			x = tx - bw;
			y = ty - bw;
			width = tw + bw + bw;
	...
	LOCAL_DEBUG_OUT ("grabbing root image from %dx%d%+d%+d", width, height,
									 x, y);

	src =
			create_visual_window (scr->asv, scr->Root, x, y, width, height, 0,
														CopyFromParent,
	...
	if (tick_count < 100)
		if ((root_im =
				 pixmap2asimage (scr->asv, src, 0, 0, width, height, AllPlanes,
												 False, 100)) != NULL) {
			if ((scr->RootClipArea.y < 0 || scr->RootClipArea.y < 0)
				...
				tmp = tile_asimage (scr->asv, root_im,
														x, y, scr->RootClipArea.width,
														scr->RootClipArea.height, TINT_NONE,
...
merge_scanlines_func mystyle_translate_texture_type (int texture_type)
{
	...
	else if (texture_type >= TEXTURE_TRANSPIXMAP
					 && texture_type < TEXTURE_TRANSPIXMAP_END)
																					 ...
																					 int height, int flip)
{
	...
	if (flip == 0)
		return im;
	tmp =
			flip_asimage (ASDefaultVisual, im, 0, 0, width, height, flip,
										ASA_ASImage, 100, ASIMAGE_QUALITY_DEFAULT);
	if (tmp != NULL) {
...
static ASImage *clip_root_pixmap (Pixmap root_pixmap, int width,
																	int height)
{
	ASImage *result = NULL;
	...
	if (root_pixmap) {
		...
		/* fprintf( stderr, "RootPixmap2RootImage %dx%d%+d%+d", root_w, root_h, root_x,
		   root_y); */
		tmp_root =
				pixmap2asimage (ASDefaultVisual, root_pixmap, root_x, root_y,
												width, height, AllPlanes, False, 100);

		LOCAL_DEBUG_OUT ("Root pixmap ASImage = %p, size = %dx%d", tmp_root,
										 tmp_root ? tmp_root->width : 0,
										 tmp_root ? tmp_root->height : 0);
		if (tmp_root) {
			if (clip_x == root_x && clip_y == root_y && clip_area->width == width
...
static ASImage *mystyle_make_image_int (MyStyle * style, int root_x,
																				int root_y, int crop_x, int crop_y,
																				int crop_width, int crop_height,
																				int scale_width, int scale_height,
																				int flip, ASImage * underlayment,
																				int overlay_type)
{
	ASImage *im = NULL;
	Pixmap root_pixmap;
	...
	if (underlayment) {
		if (overlay_type >= TEXTURE_SCALED_TRANSPIXMAP)
			overlay_type =
					TEXTURE_TRANSPIXMAP + (overlay_type -
																 TEXTURE_SCALED_TRANSPIXMAP);
		if (overlay_type < TEXTURE_TRANSPIXMAP
				|| overlay_type >= TEXTURE_TRANSPIXMAP_END)
	...
	LOCAL_DEBUG_OUT
			("style \"%s\", texture_type = %d, im = %p, tint = 0x%lX, geom=(%dx%d%+d%+d), flip = %d",
			 style->name, style->texture_type, style->back_icon.image,
			 style->tint, width, height, root_x, root_y, flip);

	if (transparency) {						/* in this case we need valid root image : */

		if (ASDefaultScr->RootImage == NULL) {
			unsigned int root_w, root_h;

			root_pixmap = ValidatePixmap (None, 1, 1, &root_w, &root_h);
			show_activity ("obtained Root pixmap = %lX, size %dx%d", root_pixmap,
										 root_w, root_h);
			ASDefaultScr->RootImage =
					clip_root_pixmap (root_pixmap, root_w, root_h);

			if (ASDefaultScr->RootImage == NULL)
				ASDefaultScr->RootImage =
						grab_root_asimage (ASDefaultScr, None, False);
		}
		LOCAL_DEBUG_OUT ("RootImage = %p clip(%ux%u%+d%+d) size(%dx%d)",
										 ASDefaultScr->RootImage,
										 ASDefaultScr->RootClipArea.width,
	...
	case TEXTURE_GRADIENT_L2R:
		{
			...
			LOCAL_DEBUG_OUT
					("orig grad type = %d, translated grad_type = %d, texture_type = %d",
					 style->gradient.type, grad->type, style->texture_type);
			im = make_gradient (ASDefaultVisual, grad, width, height, 0xFFFFFFFF,
	...
	case TEXTURE_SCALED_PIXMAP:
		if (get_flags (style->set_flags, F_SLICE)) {
			...
			im = scale_asimage (ASDefaultVisual, style->back_icon.image,
													preflip_width, preflip_height, ASA_ASImage, 0,
													ASIMAGE_QUALITY_DEFAULT);
		if (flip != 0)
	...
	case TEXTURE_PIXMAP:
		...
		LOCAL_DEBUG_OUT ("back_icon.image = %p,im = %p, preflip_size=%dx%d",
										 style->back_icon.image, im, preflip_width,
										 preflip_height);
		if (flip != 0)
			im = mystyle_flip_image (im, width, height, flip);
		break;
	...
	if (transparency) {
		if (ASDefaultScr->RootImage != NULL) {
			...
			Bool do_tint = (style->texture_type == TEXTURE_TRANSPARENT ||
											style->texture_type == TEXTURE_TRANSPARENT_TWOWAY);
			Bool do_blur = (get_flags (style->set_flags, F_BLUR)
											&& (style->blur_x > 1 || style->blur_y > 1));

			LOCAL_DEBUG_OUT ("root+im = %p, blur is %s set, size = %dx%d",
											 root_im, get_flags (style->set_flags,
																					 F_BLUR) ? "is" : "not",
											 style->blur_x, style->blur_y);
			if (do_tint
					|| (do_blur
							&& (root_src_x != 0 || root_src_y != 0
									|| width != root_im->width
									|| height != root_im->height))) {
...
#if 1
				ASImage *tmp =
						blur_asimage_gauss (ASDefaultVisual, root_im, style->blur_x,
																style->blur_y, 0xFFFFFFFF, ASA_ASImage,
			...
			if (do_tint)
				...
				layers[0].merge_scanlines =
						mystyle_merge_scanlines_func_xref[index];
				layers[0].dst_x = 0;
				layers[0].dst_y = 0;
				...
				if (get_flags (style->set_flags, F_SLICE)) {
					scaled_im = slice_asimage2 (ASDefaultVisual, layers[1].im,
																			...
																			style->slice_x_end,
																			style->slice_y_start,
																			style->slice_y_end, preflip_width,
																			preflip_height, do_scale,
																			ASA_ASImage, 0,
																			ASIMAGE_QUALITY_DEFAULT);
					...
					if (tmp) {
						if (im) {
							if (style->texture_type == TEXTURE_SHAPED_SCALED_PIXMAP ||
								...
								LOCAL_DEBUG_OUT ("copying alpha channel from %p to %p", im,
																 tmp);
			...
			show_warning
					("MyStyle \"%s\" : failed to accure Root background image",
					 style->name);
	}
	...
	if (im == NULL) {
		...
		im->back_color = style->colors.back;
		show_warning
				("MyStyle \"%s\" : failed to generate background image - using solid fill instead with color #0x%8.8X",
				 style->name, style->colors.back);
	}
	...
	if (style->overlay) {
		ASImage *overlayed =
				mystyle_make_image_int (style->overlay, root_x, root_y, crop_x,
																crop_y, crop_width, crop_height,
																scale_width, scale_height, flip, im,
																style->overlay_type);
/* 		fprintf( stderr, "overlay_style = %p\n", style->overlay ); */
	...
	if (crop_x > 0 || crop_y > 0 ||
			(crop_width > 0 && crop_width < im->width) || (crop_height > 0
																										 && crop_height <
																										 im->height)) {
...
ASImage *mystyle_make_image (MyStyle * style, int root_x, int root_y,
														 int width, int height, int flip)
{
	if (style != NULL)
		return mystyle_make_image_int (style, root_x, root_y, 0, 0, width,
																	 height, width, height, flip, NULL, 0);
	return NULL;
}

ASImage *mystyle_crop_image (MyStyle * style, int root_x, int root_y,
														 int crop_x, int crop_y, int width, int height,
														 int scale_width, int scale_height, int flip)
{
	if (style != NULL)
		return mystyle_make_image_int (style, root_x, root_y, crop_x, crop_y,
																	 width, height, scale_width,
																	 scale_height, flip, NULL, 0);
	return NULL;
}
