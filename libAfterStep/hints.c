             ...
             ASHints* reusable_memory )
{
    ...
    if( raw == NULL || list == NULL || what == 0 ) return NULL ;
                  ...
                  Window **pcmap_windows )
{
	...
	if( w == None || pcmap_windows == NULL ) return False ;
                  ...
                  ASFlagType what)
{
    ...
    {/* adding all the possible window names in order of increasing importance */
        if( raw->wm_class )
        {
            if( raw->wm_class->res_class )
                clean->res_class = add_name_to_list(clean->names,
                    stripcpy( raw->wm_class->res_class ));
            if( raw->wm_class->res_name )
                clean->res_name = add_name_to_list(clean->names,
                    stripcpy( raw->wm_class->res_name ));
        }
        if( raw->wm_icon_name )
	...
	if( get_flags(what, HINT_COLORMAP) )
    {
		if( raw->wm_cmap_windows && raw->wm_cmap_win_count > 0 )
		{
			...
			clean->cmap_windows = (Window*)safecalloc( raw->wm_cmap_win_count+1,
			                                           sizeof(Window));
...
{
    if( dst_flags == NULL || flags == 0 ) return ;
    while( (*xref)[0] != 0 )
    {
        ASFlagType to_set ;
        ASFlagType to_clear ;
...
static ASFlagsXref mwm_decor_xref[] =
{ /*Flag              Set if Set    ,Clear if Set,Set if Clear, Clear if Clear  */
...
static ASFlagsXref mwm_decor_func_xref[] =
{ /*Flag              Set if Set,  Clear if Set, Set if Clear, Clear if Clear  */
...
static ASFlagsXref mwm_func_xref[] =
{ /*Flag              Set if Set,  Clear if Set, Set if Clear, Clear if Clear  */
...
static ASFlagsXref gnome_state_xref[] =
{ /*Flag                    Set if Set,  Clear if Set, Set if Clear, Clear if Clear  */
...
static ASFlagsXref gnome_hints_xref[] =
{ /*Flag                    Set if Set,  Clear if Set, Set if Clear, Clear if Clear  */
...
Bool decode_gnome_state( ASFlagType state, ASHints *clean, ASStatusHints *status )
{
	Bool changed = False ;
...
static ASFlagsXref extwm_state_xref[] =
{ /*Flag                    Set if Set,  Clear if Set, Set if Clear, Clear if Clear  */
...
static ASFlagsXref extwm_type_xref[] =
{ /*Flag              Set if Set,      Clear if Set,     Set if Clear,    Clear if Clear  */
...
static ASFlagsXref extwm_type_func_mask[] =
{ /*Flag             Set if Set,  Clear if Set,     Set if Clear  , Clear if Clear  */
...
merge_asdb_hints (ASHints* clean, ASRawHints *raw,
                  ...
                  ASFlagType what)
{
    static ASFlagsXref asdb_startup_xref[] =
    { /*Flag                    Set if Set    ,Clear if Set    , Set if Clear, Clear if Clear  */
    ...
    static ASFlagsXref asdb_hints_xref[] =
    { /*Flag                  Set if Set      ,Clear if Set    ,Set if Clear       ,Clear if Clear  */
...
LOCAL_DEBUG_CALLER_OUT( "0x%lX", what );
	...
	if( raw == NULL || db_rec == NULL ) return ;
    if( get_flags( what, HINT_STARTUP ) && status != NULL )
    {
            ...
            !get_flags( status->flags, AS_StartPositionUser) )
        {
            ...
            if( get_flags( g->flags, XValue ) )
                status->x = get_flags(g->flags,XNegative)?
                            raw->scr->MyDisplayWidth - raw->border_width*2 - status->width - g->x :
                            g->x ;
            ...
            if( get_flags( g->flags, YValue ) )
                status->y = get_flags(g->flags,YNegative)?
                            raw->scr->MyDisplayHeight - raw->border_width*2 - status->height - g->y :
                            g->y ;
            else if(!get_flags( status->flags, AS_StartPosition ) )
                status->y = raw->placement.y ;
...
destroy_hints( ASHints *clean, Bool reusable )
{
    if( clean )
    {
        register int i ;
        for( i = 0 ; i < MAX_WINDOW_NAMES ; i++ )
            if( clean->names[i] == NULL ) break ;
            else free( clean->names[i] );
...
void
constrain_size ( ASHints *hints, ASStatusHints *status,
...
ASFlagType
change_placement( ScreenInfo * scr, ASHints *hints, ASStatusHints *status, XPoint *anchor, ASStatusHints *new_status, int vx, int vy, ASFlagType what )
{
	ASFlagType todo = 0 ;
	int grav_x, grav_y ;
...
make_client_command( ScreenInfo *scr, ASHints *hints, ASStatusHints *status, XPoint *anchor, int vx, int vy )
{
    ...
    sprintf( client_cmd,   "%s -geometry %ux%u%+d%+d -xrm \"afterstep*desk:%d\""
                           ...
                           status->layer,
                           status->viewport_x, status->viewport_y );
    return client_cmd;
}
...
serialize_string( char * string, ASVector *buf )
{
	if( buf )
	{
		...
		{
			src -= 4;
			ptr[i] = (CARD32)src[0]+0x0FFL*(CARD32)src[1]+0x0FFFFL*(CARD32)src[2]+0x0FFFFFFL*(CARD32)src[3] ;
		}
	}
}
...
serialize_clean_hints( ASHints *clean, ASVector *buf )
{
	...
	ptr[i++] = clean->icon_x;		ptr[i++] = clean->icon_y;

	ptr[i++] = clean->min_width;	ptr[i++] = clean->min_height ;
	ptr[i++] = clean->max_width;	ptr[i++] = clean->max_height ;
	ptr[i++] = clean->width_inc;	ptr[i++] = clean->height_inc ;
	ptr[i++] = clean->min_aspect.x;	ptr[i++] = clean->min_aspect.y;
	ptr[i++] = clean->max_aspect.x;	ptr[i++] = clean->max_aspect.y;
	ptr[i++] = clean->base_width;	ptr[i++] = clean->base_height ;
...
serialize_status_hints( ASStatusHints *status, ASVector *buf )
{
	...
	ptr[i++] = status->x ; 			ptr[i++] = status->y;
	ptr[i++] = status->width; 		ptr[i++] = status->height;
	ptr[i++] = status->border_width ;
	ptr[i++] = status->viewport_x;	ptr[i++] = status->viewport_y;
...
deserialize_clean_hints( CARD32 **pbuf, size_t *buf_size, ASHints *reusable_memory )
{
	...
	clean->icon_x 	 	= buf[i++]; 	clean->icon_y 		= buf[i++];

	clean->min_width 	= buf[i++]; 	clean->min_height 	= buf[i++] ;
	clean->max_width 	= buf[i++]; 	clean->max_height 	= buf[i++] ;
	clean->width_inc 	= buf[i++]; 	clean->height_inc 	= buf[i++] ;
	clean->min_aspect.x = buf[i++];		clean->min_aspect.y = buf[i++] ;
	clean->max_aspect.x = buf[i++];		clean->max_aspect.y = buf[i++] ;
	clean->base_width 	= buf[i++];		clean->base_height 	= buf[i++] ;
	clean->gravity 		= buf[i++];
...
deserialize_status_hints( CARD32 **pbuf, size_t *buf_size, ASStatusHints *reusable_memory )
{
	...
	status->flags 	= buf[i++];
	status->x 		= buf[i++];			status->y 		= buf[i++];
	status->width 	= buf[i++]; 		status->height 	= buf[i++];
	status->border_width= buf[i++] ;
	status->viewport_x 	= buf[i++] ;	status->viewport_y = buf[i++];
...
HintsTypes2Func( HintsTypes type )
{
    switch( type )
    {
        case HINTS_ICCCM : return merge_icccm_hints ;
        case HINTS_GroupLead : return merge_group_hints ;
        case HINTS_Transient : return merge_transient_hints ;
        case HINTS_Motif : return merge_motif_hints ;
        case HINTS_Gnome : return merge_gnome_hints ;
        case HINTS_ExtendedWM : return merge_extwm_hints ;
        case HINTS_XResources : return merge_xresources_hints ;
        case HINTS_ASDatabase : return merge_asdb_hints ;
        case HINTS_Supported : break;
    }
    return NULL;
}
...
{
	if( func == merge_group_hints ) return HINTS_GroupLead ;
	else if( func == merge_transient_hints ) return HINTS_Transient ;
	else if( func == merge_motif_hints ) return HINTS_Motif ;
    else if( func == merge_gnome_hints ) return HINTS_Gnome ;
    else if( func == merge_extwm_hints ) return HINTS_ExtendedWM ;
    else if( func == merge_xresources_hints ) return HINTS_XResources ;
    else if( func == merge_asdb_hints ) return HINTS_ASDatabase ;
    return HINTS_ICCCM ;
}
