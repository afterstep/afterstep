...
assimilate_rectangle (int new_x, int new_y, int new_width, int new_height, ASVector * shape)
{
	...
	for (i = 0; i < shape_rect_count; ++i)
	{
		...
		{
			LOCAL_DEBUG_OUT (" \tassimilated by %dx%d%+d%+d", shape_rects[i].width, shape_rects[i].height,
		...
		{
			if (new_x + new_width >= shape_rects[i].x && new_x <= shape_rects[i].x + shape_rects[i].width)
			{
				int           w1 = new_x + new_width;
				int           w2 = shape_rects[i].x + shape_rects[i].width;

				LOCAL_DEBUG_OUT (" \thorizontally overlapping %dx%d%+d%+d", shape_rects[i].width, shape_rects[i].height,
								 shape_rects[i].x, shape_rects[i].y);
		...
		{
			if (new_y + new_height >= shape_rects[i].y && new_y <= shape_rects[i].y + shape_rects[i].height)
			{
				int           h1 = new_y + new_height;
				int           h2 = shape_rects[i].y + shape_rects[i].height;

				LOCAL_DEBUG_OUT (" \tvertically overlapping %dx%d%+d%+d", shape_rects[i].width, shape_rects[i].height,
								 shape_rects[i].x, shape_rects[i].y);
	...
	if (selected >= 0)
	{
		...
		LOCAL_DEBUG_OUT (" \tintersected %dx%d%+d%+d", shape_rects[selected].width, shape_rects[selected].height,
						 shape_rects[selected].x, shape_rects[selected].y);
		...
		{									   /* there could be right center segment */
			if (assimilate_rectangle
				(new_x + new_width - right, new_y + top, right, new_height - (top + bottom), shape))
				changed = True;
		}
	} else
...
subtract_rectangle (int new_x, int new_y, int new_width, int new_height, ASVector * shape)
{
	...
	while (--i >= 0)
	{
		if (shape_rects[i].x >= new_x &&
			...
			shape_rects[i].y + shape_rects[i].height <= new_y + new_height)
		{
			LOCAL_DEBUG_OUT (" \ttrashing %dx%d%+d%+d", shape_rects[i].width, shape_rects[i].height, shape_rects[i].x,
							 shape_rects[i].y);
...
Bool
add_shape_rectangles (ASVector * shape, XRectangle * rects, unsigned int count, int x_origin, int y_origin,
					  unsigned int clip_width, unsigned int clip_height)
{
	...
	print_shape (shape);
#endif
	LOCAL_DEBUG_OUT ("adding %d rectangles at %+d%+d clipped by %dx%d", count, x_origin, y_origin, clip_width,
					 clip_height);
	...
	for (i = 0; i < count; ++i)
	{
		...
		new_height =
			(new_y + (int)rects[i].height > (int)clip_height) ? (int)clip_height - new_y : (int)rects[i].height;
...
print_shape (ASVector * shape)
{
	if (shape)
	{
		...
		for (i = 0; i < shape_rect_count; ++i)
			show_progress ("\trects[%d] = %dx%d%+d%+d;", i, shape_rects[i].width, shape_rects[i].height,
						   shape_rects[i].x, shape_rects[i].y);
		return True;
...
Bool
subtract_shape_rectangle (ASVector * shape, XRectangle * rects, unsigned int count, int x_origin, int y_origin,
						  unsigned int clip_width, unsigned int clip_height)
{
	int           i;
	int           new_x, new_y, new_width, new_height;
	...
	LOCAL_DEBUG_OUT ("subtract %d rectangles at %+d%+d clipped by %dx%d", count, x_origin, y_origin, clip_width,
					 clip_height);
	for (i = 0; i < count; ++i)
	{
		new_x = rects[i].x + x_origin;
		...
		{									   /* we may need to create 2 vertical rectangles ( left and right ) : */
			...
			if (right > r_left && right < r_right)
			{
				...
				if (disected)
				{
					...
					rects = PVECTOR_HEAD (XRectangle, list);	/* memory may have gotten reallocated */
					LOCAL_DEBUG_OUT ("added rect - (%d,%d)-(%d,%d)", tmp.x, tmp.y, tmp.x + tmp.width,
									 tmp.y + tmp.height);
				} else
				{
					rects[i] = tmp;
					disected = True;
					LOCAL_DEBUG_OUT ("adjusted rect[%d] - (%d,%d)-(%d,%d)", i, tmp.x, tmp.y, tmp.x + tmp.width,
									 tmp.y + tmp.height);
				}
			}
		}
