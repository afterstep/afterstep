...
make_color_scheme_argb (ASColorScheme * cs, int id, CARD32 base_alpha16, CARD32 hue360, CARD32 sat100, CARD32 val100)
{
	...
	argb = MAKE_ARGB32_CHAN16 (base_alpha16, ARGB32_ALPHA_CHAN) |
		...
		MAKE_ARGB32_CHAN16 (green16, ARGB32_GREEN_CHAN) | MAKE_ARGB32_CHAN16 (blue16, ARGB32_BLUE_CHAN);
...
make_color_scheme_hsv (ARGB32 argb, int *phue, int *psat, int *pval)
{
	...
	if (ARGB32_RED16 (argb) == ARGB32_GREEN16 (argb) && ARGB32_GREEN16 (argb) == ARGB32_BLUE16 (argb))
	{
		if (phue)
			*phue = -1;
		if (psat)
...
is_light_hsv (int hue360, int sat100, int val100)
{
	...
	hsv2rgb (degrees2hue16 (hue360), percent2val16 (sat100), percent2val16 (val100), &red16, &green16, &blue16);
	show_progress ("hsv(%d,%d,%d)->rgb16(%d,%d,%d)->crit%d", hue360, sat100, val100, red16, green16, blue16,
				   ASCS_BLACK_O_WHITE_CRITERIA16_VAL (red16, green16, blue16));
...
compare_color_lightness (ARGB32 c1, ARGB32 c2)
{
	...
	return ASCS_BLACK_O_WHITE_CRITERIA16_VAL (red1, green1, blue1) - ASCS_BLACK_O_WHITE_CRITERIA16_VAL (red2, green2,
																										blue2);
}
...
static inline ARGB32
MAKE_ARGB32_SHADE100 (CARD32 a, int s100)
{
...
int
make_text_shade (int background_shade)
{
...
make_mono_ascolor_scheme (ARGB32 base)
{
	...
	make_mono_grad_argb (&(cs->main_colors[ASMC_ActiveDark]), base_alpha16, shade, &(cs->main_values[ASMC_ActiveDark]));
	active_val = shade;
	shade = make_text_shade (shade);
...
make_ascolor_scheme (ARGB32 base, int angle)
{
	...
	make_color_scheme_argb (cs, ASMC_InactiveText1, base_alpha16, inactive_text1_hue, inactive_text1_sat,
							inactive_text1_val);
	...
	make_color_scheme_argb (cs, ASMC_InactiveText2, base_alpha16, inactive_text2_hue, inactive_text2_sat,
							inactive_text2_val);
	...
	make_color_scheme_argb (cs, ASMC_HighInactive, base_alpha16, inactive1_hue, inactive1_sat,
							inactive1_val + ASCS_HIGH_BRIGHTNESS_OFFSET);
	make_color_scheme_argb (cs, ASMC_HighActive, base_alpha16, active_hue, active_sat,
							active_val + ASCS_HIGH_BRIGHTNESS_OFFSET);
	make_color_scheme_argb (cs, ASMC_HighInactiveBack, base_alpha16, base_hue, base_sat,
							base_val + ASCS_HIGH_BRIGHTNESS_OFFSET);
	make_color_scheme_argb (cs, ASMC_HighActiveBack, base_alpha16, active_hue, active_sat,
							active_val - ASCS_HIGH_BRIGHTNESS_OFFSET);
	/* active hue */
	...
	make_color_scheme_argb (cs, ASMC_HighInactiveText, base_alpha16, active_hue, high_inactive_text_sat,
	...
	make_color_scheme_argb (cs, ASMC_HighActiveText, base_alpha16, base_hue, high_active_text_sat,
							high_active_text_val);

#if 0
	if (cs->base_sat > ASCS_DISABLED_SATURATION_LEVEL)
		cs->main_colors[ASMC_DisabledText] =
			make_color_scheme_argb (base_alpha16, cs->base_hue, cs->base_sat - ASCS_DISABLED_SATURATION_LEVEL,
									cs->high_inactive_text_val);
	else if (cs->base_sat > ASCS_DISABLED_SATURATION_THRESHOLD)
		cs->main_colors[ASMC_DisabledText] =
			make_color_scheme_argb (base_alpha16, cs->base_hue, cs->base_sat + (ASCS_DISABLED_SATURATION_LEVEL * 2),
									cs->high_inactive_text_val);
	else
		cs->main_colors[ASMC_DisabledText] =
			make_color_scheme_argb (base_alpha16, cs->base_hue, cs->base_sat + ASCS_DISABLED_SATURATION_LEVEL,
...
#endif
	if (base_val > 50)
		make_color_scheme_argb (cs, ASMC_DisabledText, base_alpha16, base_hue, (base_sat * 80) / 100,
								base_val - ASCS_GRADIENT_BRIGHTNESS_OFFSET / 2);
	else
		make_color_scheme_argb (cs, ASMC_DisabledText, base_alpha16, base_hue, (base_sat * 80) / 100,
								base_val - ASCS_GRADIENT_BRIGHTNESS_OFFSET / 2);
	...
	make_grad_argb (cs, ASMC_HighInactiveDark, base_alpha16, inactive1_hue, inactive1_sat,
					inactive1_val + ASCS_HIGH_BRIGHTNESS_OFFSET, False);
	make_grad_argb (cs, ASMC_HighActiveDark, base_alpha16, active_hue, active_sat,
					active_val + ASCS_HIGH_BRIGHTNESS_OFFSET, False);
	make_grad_argb (cs, ASMC_HighInactiveBackDark, base_alpha16, base_hue, base_sat,
					base_val + ASCS_HIGH_BRIGHTNESS_OFFSET, False);
	make_grad_argb (cs, ASMC_HighActiveBackDark, base_alpha16, active_hue, active_sat,
					active_val - ASCS_HIGH_BRIGHTNESS_OFFSET, False);
