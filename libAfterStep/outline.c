...
make_fvwm_segments (ASOutlineSegment * s, struct MRRectangle *geom, unsigned int scr_width, unsigned int scr_height)
{
	...
	/* Inside grid : */
	count +=
		make_rectangle_segments (s ? &(s[count]) : NULL, geom->x + 1, geom->y + geom->height / 3, geom->width - 2,
								 geom->height / 3);
	count +=
		make_rectangle_segments (s ? &(s[count]) : NULL, geom->x + geom->width / 3, geom->y + 1, geom->width / 3,
								 geom->height - 2);
	return count;
}
...
make_tek_segments (ASOutlineSegment * s, struct MRRectangle *geom, unsigned int scr_width, unsigned int scr_height)
{
	...
	count += make_rectangle_segments (s, geom->x, geom->y, geom->width, geom->height);
	count +=
		make_lt_corner_segments (s ? &(s[count]) : NULL, geom->x - 6, geom->y - 6, geom->width / 2, geom->height / 2);
	count +=
		make_br_corner_segments (s ? &(s[count]) : NULL, geom->x + geom->width + 6, geom->y + geom->height + 6,
...
make_tek2_segments (ASOutlineSegment * s, struct MRRectangle *geom, unsigned int scr_width, unsigned int scr_height)
{
	...
	count += make_rectangle_segments (s, geom->x, geom->y, geom->width, geom->height);
	count +=
		make_lt_corner_segments (s ? &(s[count]) : NULL, geom->x - 12, geom->y - 12, geom->width / 3, geom->height / 3);
	count +=
		make_lt_corner_segments (s ? &(s[count]) : NULL, geom->x - 6, geom->y - 6, geom->width / 2, geom->height / 2);
	count +=
		make_br_corner_segments (s ? &(s[count]) : NULL, geom->x + geom->width + 6, geom->y + geom->height + 6,
								 geom->width / 2, geom->height / 2);
	count +=
		make_br_corner_segments (s ? &(s[count]) : NULL, geom->x + geom->width + 12, geom->y + geom->height + 12,
...
make_corner_segments (ASOutlineSegment * s, struct MRRectangle *geom, unsigned int scr_width, unsigned int scr_height)
{
	...
	count +=
		make_br_corner_segments (s ? &(s[count]) : NULL, geom->x + geom->width, geom->y + geom->height, corner_width,
								 corner_height);
...
make_outline_segments (ASWidget * parent, MyLook * look)
{
	...
	for (i = count - 1; i >= 0; --i)
	{
		s[i].w = create_screen_window (AS_WIDGET_SCREEN (parent), AS_WIDGET_WINDOW (parent), -20, -20, 10, 1, 1,	/* 1 pixel wide border */
									   InputOutput, (CWBackPixel | CWBorderPixel | CWSaveUnder | CWEventMask), &attr);
