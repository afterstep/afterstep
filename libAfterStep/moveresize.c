...
void update_ashint_geometry (ASHintWindow * hw, Bool force_redraw)
{
	...
	if (get_flags (hw->look->resize_move_geometry.flags, XValue | YValue) !=
			(XValue | YValue))
...
ASHintWindow *create_ashint_window (ScreenInfo * scr, struct MyLook *look,
																		const char *hint)
{
	...
	set_astbar_style (hw->bar, BAR_STATE_UNFOCUSED,
										look->MSWindow[BACK_FOCUSED]->name);

	add_astbar_label (hw->bar, 0, 0, 0, ALIGN_CENTER, 1, 1, " ",
										AS_Text_ASCII);
	attr.override_redirect = True;
	attr.save_under = True;
	hw->w =
			create_screen_window (scr, None, 0, 0, 1, 1, 0, InputOutput,
...
typedef int (*moveresize_event_func) (struct ASMoveResizeData *,
																			struct ASEvent * event);

Bool (*_as_grab_screen_func) (struct ScreenInfo * scr, Cursor cursor) =
		NULL;
 ...
 */
static ASEvent _as_pressed_buttons[5] =
...
/**********************************************************************/
Bool
start_widget_moveresize (ASMoveResizeData * data,
												 moveresize_event_func handler)
{
...
Bool check_moveresize_event (ASEvent * event)
{
	LOCAL_DEBUG_OUT ("data(%p)->handler(%p)", _as_curr_moveresize_data,
									 _as_curr_moveresize_handler);
	if (_as_curr_moveresize_data)
		if (_as_curr_moveresize_handler (_as_curr_moveresize_data, event) ==
				ASE_Consumed)
			return True;
	return False;
}
...
grab_widget_pointer (ASWidget * widget, ASEvent * trigger,
										 ...
										 int *x_return, int *y_return, int *root_x_return,
										 int *root_y_return, unsigned int *mask_return)
{
	int i;
	Window wjunk;
	...
	if (widget == NULL)
		return False;
	ASQueryPointerWinXYMask (AS_WIDGET_WINDOW (widget), root_x_return,
													 root_y_return, x_return, y_return, mask_return);
	LOCAL_DEBUG_OUT
			("grabbing pointer at window(%+d%+d) root(%+d%+d), mask = 0x%X, window(%lX)",
			 *x_return, *y_return, *root_x_return, *root_y_return, *mask_return,
			 AS_WIDGET_WINDOW (widget));
	if (trigger) {
		if (trigger->scr)
			scr = trigger->scr;
	}
...
/*	XUngrabPointer( dpy, trigger->event_time ); */
	if (_as_grab_screen_func == NULL
		...
		for (i = 0; i < 5; ++i) {
			SHOW_CHECKPOINT;
			LOCAL_DEBUG_OUT ("comparing mask = 0x%X and 0x%X", *mask_return,
											 (Button1Mask << i));
			if (get_flags (*mask_return, (Button1Mask << i))) {
				LOCAL_DEBUG_OUT ("pressed widget[%d] = %p and widget = %p", i,
				...
				if (_as_pressed_buttons[i].widget != NULL
					...
					XTranslateCoordinates (dpy, AS_WIDGET_SCREEN (tmp.widget)->Root,
																 ...
																 *x_return, *y_return, &(tmp.x.xbutton.x),
																 &(tmp.x.xbutton.y), &wjunk);
					tmp.x.xbutton.state = *mask_return;
#ifdef AS_DISPATCHER_H_HEADER_INCLUDED	/* in as-devel */
					handle_widgets_event (&tmp);
#else
					XSendEvent (dpy, tmp.x.xany.window, False, ButtonReleaseMask,
											&(tmp.x));
#endif
				}
			}
 ...
 ***********************************************************************/
static void
prepare_move_resize_data (ASMoveResizeData * data, ASWidget * parent,
													ASWidget * mr, ASEvent * trigger,
													...
													int opaque_size)
{
	...
	if (apply_func != NULL &&
			...
			(opaque_size * AS_WIDGET_WIDTH (parent) *
			 AS_WIDGET_HEIGHT (parent)) / 100)
	...
	data->start.x = data->curr.x = data->last.x =
			AS_WIDGET_X (mr) - AS_WIDGET_X (parent);
	data->start.y = data->curr.y = data->last.y =
			AS_WIDGET_Y (mr) - AS_WIDGET_Y (parent);
	data->start.width = data->curr.width = data->last.width =
			AS_WIDGET_WIDTH (mr);
	data->start.height = data->curr.height = data->last.height =
			AS_WIDGET_HEIGHT (mr);
	...
	grab_widget_pointer (parent, trigger,
											 ...
											 LeaveWindowMask, &(data->last_x), &(data->last_y),
											 &root_x, &root_y, &(data->pointer_state));

	data->stop_on_button_press =
			((data->pointer_state & ButtonAnyMask) == 0);
	...
	/* " %u x %u %+d %+d " */
	data->geometry_string =
			safemalloc (1 + 6 + 3 + 6 + 2 + 6 + 2 + 6 + 1 + 1 +
									30 /*for the heck of it */ );
	data->geometry_display =
			create_ashint_window (scr, look,
														"88888 x 88888 +88888 +88888 (-88888 -88888)");

	data->below_sibling = data->geometry_display->w;

	LOCAL_DEBUG_OUT ("curr(%+d%+d)->last(%+d%+d)", data->curr.x,
									 data->curr.y, data->last_x, data->last_y);

	if (data->pointer_func != NULL)
		data->pointer_func (data, data->last_x, data->last_y);
}

static void update_geometry_display (ASMoveResizeData * data)
{
	...
	if (data->geom_y_mult > 0 && data->geom_y_div > 0) {
		...
		display_height =
				(display_height * data->geom_y_mult) / data->geom_y_div;
	}
	east_x =
			display_x + display_width -
			data->geometry_display->scr->MyDisplayWidth;
	south_y =
			display_y + display_height -
			data->geometry_display->scr->MyDisplayHeight;
	if (data->width_inc != 0)
		display_width /= (int)data->width_inc;
	if (data->height_inc != 0)
		display_height /= (int)data->height_inc;
	if (display_width == 0 && display_height != 0)
		sprintf (data->geometry_string, "x %d %+d %+d", display_height,
						 display_x, display_y);
	else if (display_width != 0 && display_height == 0)
		sprintf (data->geometry_string, "%d x  %+d %+d", display_width,
						 display_x, display_y);
	else if (display_width != 0 && display_height != 0)
		sprintf (data->geometry_string, "%d x %d %+d %+d (%+d %+d)",
						 display_width, display_height, display_x, display_y, east_x,
						 south_y);
	else
...
static void flush_move_resize_data (ASMoveResizeData * data)
{
...
void complete_interactive_action (ASMoveResizeData * data, Bool cancel)
{
	...
	if (!cancel && data->pointer_func == move_func) {
		...
		XSendEvent (dpy, client_event.xconfigure.event, False, 0,
								&client_event);
	}
...
Bool move_resize_loop (ASMoveResizeData * data, ASEvent * event)
{
	Bool finished = False;
	int new_x, new_y;
	...
	if (event->eclass & ASE_POINTER_EVENTS) {
		...
		if (event->x.type == MotionNotify) {
			...
			if (data->stop_on_button_press) {
				...
				SHOW_CHECKPOINT;
			} else if (!get_flags (event->x.xmotion.state, AllButtonMask)) {	/* all the buttons are depressed !!! */
	...
	SHOW_CHECKPOINT;
	switch (event->x.type) {			/* Handle a limited number of key press events to allow mouseless
																 * operation */
	...
	case KeyPress:
		{
			...
			XWarpPointer (dpy, Scr.Root, Scr.Root, 0, 0, Scr.MyDisplayWidth,
										Scr.MyDisplayHeight, new_x, new_y);
	...
	case ButtonRelease:
		SHOW_CHECKPOINT;
		if ((data->stop_on_button_press ? 1 : 0) ==
				(event->x.type == ButtonPress ? 1 : 0))
			finished = True;
	case MotionNotify:
...
/* at best we can swallow couple MotionNotify events - but care should be taken even here ! */
			...
			LOCAL_DEBUG_OUT ("new = %+d%+d, finished = %d", new_x, new_y,
											 finished);
		}
...
void move_func (struct ASMoveResizeData *data, int x, int y)
{
	...
	data->last_y = y;
	LOCAL_DEBUG_OUT
			("last_geom(%+d%+d)->curr_geom(%+d%+d)->delta(%+d%+d)->lag(%+d%+d)->last(%+d%+d)",
			 data->last.x, data->last.y, data->curr.x, data->curr.y, dx, dy,
...
void resize_func (struct ASMoveResizeData *data, int x, int y)
{
	...
	case FR_NW:
		real_dy =
				adjust_west_side (data->grid, h_lines, dy, &(data->curr.y),
													&(data->curr.height), lim1, lim2,
													data->title_north, data->title_west);
		real_dy =
				restrain_west_side (real_dy, &(data->curr.y), &(data->curr.height),
														data->min_height, data->height_inc,
														data->max_height);
		break;
	...
	case FR_SW:
		real_dy =
				adjust_east_side (data->grid, h_lines, dy, data->curr.y,
													&(data->curr.height), lim1, lim2,
													data->title_west);
		real_dy =
				restrain_east_side (real_dy, &(data->curr.height),
														data->min_height, data->height_inc,
														data->max_height);
		break;
	}
	...
	case FR_SE:
		real_dx =
				adjust_east_side (data->grid, v_lines, dx, data->curr.x,
													&(data->curr.width), lim1, lim2,
													data->title_west);
		real_dx =
				restrain_east_side (real_dx, &(data->curr.width), data->min_width,
														data->width_inc, data->max_width);
		break;
	case FR_W:
	case FR_SW:
	case FR_NW:
		real_dx =
				adjust_west_side (data->grid, v_lines, dx, &(data->curr.x),
													&(data->curr.width), lim1, lim2,
													data->title_west, data->title_north);
		real_dx =
				restrain_west_side (real_dx, &(data->curr.x), &(data->curr.width),
														data->min_width, data->width_inc,
														data->max_width);
		break;
	}
...
/* 	data->lag_y = real_dy - dy ; */
	...
	if (data->curr.x != data->last.x || data->curr.y != data->last.y ||
			data->curr.width != data->last.width
			|| data->curr.height != data->last.height)
		data->apply_func (data);
	update_geometry_display (data);

	LOCAL_DEBUG_OUT (" dx = %d, width  = %d, x = %d, lag_x = %d", dx,
									 data->curr.width, data->curr.x, data->lag_x);
	LOCAL_DEBUG_OUT (" dy = %d, height = %d, y = %d, lag_y = %d", dy,
									 data->curr.height, data->curr.y, data->lag_y);
/*  fprintf( stderr, "move_func: (x,y) =(%d,%d) to %+d%+d\n", x, y, pdata->new_x, pdata->new_y );
*/
/*	resist_move (pdata); */
}
...
ASMoveResizeData *move_widget_interactively (ASWidget * parent,
																						 ASWidget * mr,
																						 ASEvent * trigger,
																						 as_interactive_apply_handler
																						 apply_func,
																						 as_interactive_complete_handler
																						 complete_func)
{
	...
	if (!start_widget_moveresize (data, move_resize_loop)) {
		...
		prepare_move_resize_data (data, parent, mr, trigger, apply_func,
															complete_func,
...
ASMoveResizeData *resize_widget_interactively (ASWidget * parent,
																							 ASWidget * mr,
																							 ASEvent * trigger,
																							 as_interactive_apply_handler
																							 apply_func,
																							 as_interactive_complete_handler
																							 complete_func, int side)
{
	ASMoveResizeData *data = safecalloc (1, sizeof (ASMoveResizeData));
	...
	if (!start_widget_moveresize (data, move_resize_loop)) {
		...
		LOCAL_DEBUG_OUT ("requested side = %d, using side = %d", side,
										 data->side);
	}
	return data;
}
...
void
set_moveresize_restrains (ASMoveResizeData * data, ASHints * hints,
													ASStatusHints * status)
{
	if (data) {
		...
		if (status) {
			data->frame_width =
					status->frame_size[FR_W] + status->frame_size[FR_E];
			data->frame_height =
					status->frame_size[FR_N] + status->frame_size[FR_S];
...
void
set_moveresize_aspect (ASMoveResizeData * data, int x_mult, int x_div,
											 int y_mult, int y_div, int x_origin, int y_origin)
{
	if (data) {
