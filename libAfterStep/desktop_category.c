...
int
ref_desktop_category (ASDesktopCategory * dc)
{
	if (dc)
		return ++dc->ref_count;
	return 0;
...
int
unref_desktop_category (ASDesktopCategory * dc)
{
	if (dc)
	{
		LOCAL_DEBUG_OUT ("dc = %p, ref_count = %d", dc, dc->ref_count);
...
ASDesktopEntryInfo *
desktop_category_get_entries (ASCategoryTree * ct, ASDesktopCategory * dc, int max_depth, ASHashTable * exclusions,
							  int *nitems)
{
...
Bool
desktop_entry_in_subcategory (ASCategoryTree * ct, ASDesktopEntry * de, ASDesktopEntryInfo * entries, int entries_num)
{
	int           k;
...
ASDesktopEntry *
create_desktop_entry (ASDesktopEntryTypes default_type)
{
...
int
ref_desktop_entry (ASDesktopEntry * de)
{
	if (de)
		return ++de->ref_count;
	return 0;
...
int
unref_desktop_entry (ASDesktopEntry * de)
{
	if (de)
	{
		if ((--de->ref_count) > 0)
...
DupDesktopEntryVal_func (Name) DupDesktopEntryVal_func (Comment) FunctionCode
...
Bool
desktop_entry_belongs_to (ASCategoryTree * ct, ASDesktopEntry * de, ASDesktopEntry * category_de)
{
...
Bool
register_desktop_entry (ASCategoryTree * ct, ASDesktopEntry * de)
{
	int           i;
		...
		if (existing_de != NULL && existing_de->type == ASDE_TypeDirectory)
		{
			...
			if (index_name)
			{
#if defined(LOCAL_DEBUG) && !defined(NO_DEBUG_OUTPUT)
				{
					...
					LOCAL_DEBUG_OUT ("entry %p added under non-standard index_name = \"%s\", fetch returned = %p", de,
									 index_name, ttt);
				}
#endif
				ref_desktop_entry (de);
	...
	if (de->categories_num == 0 || mystrcasecmp (de->categories_shortcuts[0], DEFAULT_DESKTOP_CATEGORY_NAME) == 0)
	{
		top_level = True;
	}
	...
	if (de->type == ASDE_TypeDirectory)
	{
		...
		if (dc != NULL)
		{
			LOCAL_DEBUG_OUT ("category already exists with index_name \"%s\" and name \"%s\"", dc->index_name,
	...
	for (i = top_level ? 1 : 0; i < de->categories_num; ++i)
	{
		...
		if (de->categories_num == 1 || mystrcasecmp (ct->name, de->categories_shortcuts[i]) != 0)
...
add_category_tree_subtree (ASCategoryTree * ct, ASCategoryTree * subtree)
{
	...
	if (start_hash_iteration (subtree->categories, &i))
	{
		do
		{
			...
			LOCAL_DEBUG_OUT ("category with index_name \"%s\" and name \"%s\" exists as %p", subtree_dc->index_name,
			...
			if (dc == NULL)
			{
				...
				LOCAL_DEBUG_OUT ("creating category using index_name \"%s\" and name \"%s\"", dc->index_name,
								 dc->name ? dc->name : "(null)");
				if (add_hash_item (ct->categories, AS_HASHABLE (dc->index_name), dc) != ASH_Success)
				{
					unref_desktop_category (dc);
