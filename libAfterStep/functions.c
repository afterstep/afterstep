...
init_list_of_funcs (struct ASHashTable **list, Bool force)
{
	...
	if (*list == NULL)
		*list = create_ashash (0, casestring_hash_value, casestring_compare, complex_function_destroy);
}
...
{
#ifdef DEBUG_ALLOCS
	LOCAL_DEBUG_CALLER_OUT ("menu_data_item_destroy(\"%s\",%p)", ((mdi && mdi->fdata) ? (mdi->fdata->name) : "NULL"),
							mdi);
#endif
	if (mdi)
	{
...
load_menuitem_pmap (MenuDataItem * mdi, MinipixmapTypes type, Bool force)
{
	...
	if (filename)
	{
		...
		if (func == F_CHANGE_BACKGROUND || func == F_CHANGE_BACKGROUND_FOREIGN)
		{
			...
			if (use_thumbnail)
				minipixmap->image =
					get_thumbnail_asimage (ASDefaultScr->image_manager, filename, 0, h,
										   AS_THUMBNAIL_PROPORTIONAL | AS_THUMBNAIL_DONT_ENLARGE);
...
print_func_data (const char *file, const char *func, int line, FunctionData * data)
{
	...
	else
	{
		...
		{
			fprintf (stderr, "%s \"%s\" text[%s] ", term->keyword, data->name ? data->name : "",
					 data->text ? data->text : "");
			fprintf (stderr, "val0[%ld%c(%ld)] ", (long)data->func_val[0],
					 (data->unit[0] == '\0') ? ' ' : data->unit[0], (long)data->unit_val[0]);
			fprintf (stderr, "val1[%ld%c(%ld)] ", (long)data->func_val[1],
					 (data->unit[1] == '\0') ? ' ' : data->unit[1], (long)data->unit_val[1]);
...
char         *
format_FunctionData (FunctionData * data)
{
	char         *buffer = NULL;
