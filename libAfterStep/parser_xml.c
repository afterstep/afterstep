	...
	if (pterm->type == TT_COMMENT || pterm->type == TT_ANY)
	{
		...
		if (config->current_data_len > 0)
		{
			...
			else
			{
				...
				sprintf (cdata, get_flags (config->current_flags, CF_PUBLIC_OPTION) ? "*%s %s" : "%s %s", token,
						 config->current_data);
				free (token);
			}
...
xml_tree2file (FILE * fp, char *myname, SyntaxDef * syntax, xml_elem_t * tree, int level)
{
	...
	for (;; tree = tree->next)
	{
				...
				for (c = parm_tree; c; c = c->next)
				{
					if (strcmp (c->tag, ASXMLOpt_moduleAttrName) == 0)
						tag_myname = c->parm;
							 ...
							 strcmp (c->tag, ASXMLOpt_sideAttrName) == 0 || strcmp (c->tag, ASXMLOpt_nameAttrName) == 0)
						prefix = c->parm;
				}
			}
			if (tag_myname == NULL)
		...
		if (tree)
		{
			if (syntax->terminator != '\0' && (sub_terminator == ' ' || sub_terminator != syntax->terminator))
			{
...
Bool
xml_opt_tree_insert (ASXmlOptionTreeContext * context, int term_id, Bool module_specific, ASXmlOptIDType id_type,
...
Bool
xml_opt_tree_copy (ASXmlOptionTreeContext * context, Bool module_specific, ASXmlOptIDType id_type, const char *id,
				   const char *value)
{
	if (context == NULL || context->current == NULL)
		return False;
