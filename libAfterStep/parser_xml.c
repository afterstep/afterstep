...
void append_asconfig_value_tag (xml_elem_t * parent, int type, char **val)
{
...
void statement2xml_elem (ConfigDef * config)
{
	...
	*tail = item;
	...
	if (pterm->type == TT_COMMENT || pterm->type == TT_ANY) {
		...
		if (config->current_data_len > 0) {
			...
			if (pterm->type == TT_COMMENT)
				...
				cdata =
						safemalloc (1 + strlen (token) + 1 + config->current_data_len +
												1);
				sprintf (cdata,
								 get_flags (config->current_flags,
														CF_PUBLIC_OPTION) ? "*%s %s" : "%s %s", token,
								 config->current_data);
				free (token);
			}
	...
	item->tag =
			pterm->keyword[0] ? mystrdup (pterm->keyword) : mystrdup ("item");
	LOCAL_DEBUG_OUT ("adding storage ... pterm is \"%s\" (%s)", item->tag,
									 get_flags (pterm->flags, TF_NAMED) ? "named" : "");
	...
	if (get_flags (config->current_flags, CF_FOREIGN_OPTION)) {
		...
		if (token_len > 0) {
			if (token_len > pterm->keyword_len && pterm->keyword_len > 0)
				if (mystrncasecmp
						(&module[token_len - pterm->keyword_len], pterm->keyword,
						 pterm->keyword_len) == 0)
	...
	if (name)
		parm_len =
				sizeof (" " ASXMLOpt_nameAttrName "=") + 1 + strlen (name) + 1 + 1;
	if (side)
		parm_len +=
				sizeof (" " ASXMLOpt_sideAttrName "=") + 1 + strlen (side) + 1 + 1;
	if (index)
		parm_len +=
				sizeof (" " ASXMLOpt_indexAttrName "=") + 1 + strlen (index) + 1 +
				1;
	if (module)
		parm_len +=
				sizeof (" " ASXMLOpt_moduleAttrName "=") + 1 + strlen (module) +
				1 + 1;
	...
	if (parm_len > 0) {
		...
		if (module) {
			sprintf (parm_ptr,
							 (parm_ptr > item->parm) ? " %s=\"%s\"" : "%s=\"%s\"",
							 ASXMLOpt_moduleAttrName, module);
...
//  pNext->flags = config->current_flags;
	...
	if (ptr && ptr[0]) {					/* add value as CDATA */
...
Bool
xml_tree2file (FILE * fp, char *myname, SyntaxDef * syntax,
							 xml_elem_t * tree, int level)
{
	...
	for (;; tree = tree->next) {
		...
		if (tree == NULL) {					/* need to check for syntax terminator */
			...
			if (pterm == NULL) {
				...
				if (get_hash_item
						(syntax->term_hash, AS_HASHABLE (tree->tag),
			...
			if (prefix) {
				if (get_flags (pterm->flags, TF_NAMED))
					fprintf (fp, " \"%s\"", prefix);
								 ...
								 (pterm->flags, TF_INDEXED | TF_DIRECTION_INDEXED))
					fprintf (fp, " %s", prefix);
			}
			...
			if (pterm->sub_syntax) {
				...
				if (child) {
					if (pterm->sub_syntax->terminator == '\n')
						fputc ('\n', fp);
					xml_tree2file (fp, tag_myname, pterm->sub_syntax, child,
												 level + 1);
		...
		if (tree) {
			if (syntax->terminator != '\0'
					&& (sub_terminator == ' '
							|| sub_terminator != syntax->terminator)) {
...
xml_elem_t *file2xml_tree (const char *filename, char *myname,
													 SyntaxDef * syntax)
{
	ConfigData cd;
	...
	config_reader =
			InitConfigReader (myname, syntax, CDT_Filename, cd, NULL);
...
static void parse_curr_opt_attributes (ASXmlOptionTreeContext * context)
{
	...
	for (c = parms; c != NULL; c = c->next) {
		if (mystrcasecmp (c->tag, ASXMLOpt_moduleAttrName) == 0) {
			...
			if (context->module[0] == '\0'
					|| strcmp (c->parm, context->module) == 0)
...
static void find_next_valid_opt (ASXmlOptionTreeContext * context)
{
...
ASXmlOptionTreeContext *xml_opt_tree_start_traversing (SyntaxDef * syntax,
																											 xml_elem_t **
																											 pcontainer,
																											 xml_elem_t *
																											 subtree,
...
ASXmlOptionTreeContext *file2xml_tree_context (const char *filename,
																							 char *myname,
																							 SyntaxDef * syntax)
{
...
Bool xml_opt_tree_delete_current (ASXmlOptionTreeContext * context)
{
	if (context == NULL || context->current == NULL)
		return False;
...
Bool xml_opt_tree_delete_subtree (ASXmlOptionTreeContext * context)
{
	if (context == NULL || context->current == NULL)
		return False;
...
Bool
xml_opt_tree_insert (ASXmlOptionTreeContext * context, int term_id,
										 Bool module_specific, ASXmlOptIDType id_type,
...
Bool
xml_opt_tree_copy (ASXmlOptionTreeContext * context, Bool module_specific,
									 ASXmlOptIDType id_type, const char *id,
									 const char *value)
{
	if (context == NULL || context->current == NULL)
		return False;
