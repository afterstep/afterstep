...
/******************** Window Manager Selection  ***************************/ 
		XErrorHandler  catch_redirect_error (Display * dpy,
																					XErrorEvent * event) 
{
...
}  Bool  accure_wm_selection (ASWMProps * wmprops) 
{
		...
		unsigned int udumm;
				...
				XSetErrorHandler ((XErrorHandler) catch_redirect_error);
...
 Bool  setup_volitile_wmprops (ASWMProps * wmprops) 
{
	...
	set_text_property (w, _XA_NET_WM_NAME, &_as_afterstep_name, 1,
											TPE_UTF8);
...
 Bool  read_as_current_viewport (ASWMProps * wmprops, Bool deleted) 
{
					 ...
					 (wmprops->scr->Root, _AS_CURRENT_VIEWPORT, 2, &raw_data,
						&nitems))
...
#define WMP_ClientWritable  (0x01<<1)
	...
	&_XA_NET_CURRENT_DESKTOP, read_extwm_current_desk, WMC_DesktopCurrent,
				0},  {
...
NULL, NULL, 0, 0} };
	...
	&_AS_BACKGROUND, read_as_background, WMC_ASBackgrounds,
				WMP_ClientWritable},  {
...
 void  read_volitile_wmprops (ASWMProps * wmprops) 
{
			...
			XListProperties (dpy, wmprops->selection_window, &props_num);
...
 ASWMProps * setup_wmprops (ScreenInfo * scr, Bool manager,
															 ASFlagType what,
															 ASWMProps * reusable_memory) 
{
...
 Bool  set_current_viewport_prop (ASWMProps * wmprops, CARD32 vx,
...
 Bool  set_desktop_geometry_prop (ASWMProps * wmprops, CARD32 width,
																		 CARD32 height) 
{
...
  void  flush_wmprop_data (ASWMProps * wmprops, ASFlagType what) 
{
			...
			for (i = 0; WMPropsDescriptions_volitile[i].id_variable != NULL;
						i++)
...
/********************************************************************************/ 
		WMPropClass  handle_wmprop_event (ASWMProps * wmprops,
																			 XEvent * event) 
{
