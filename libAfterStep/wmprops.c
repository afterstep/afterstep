...
}  Bool  accure_wm_selection (ASWMProps * wmprops) 
{
	...
	{										   /* now we need to obtain a valid timestamp : */
		...
		 XChangeProperty (dpy, w, wmprops->_XA_WM_S, wmprops->_XA_WM_S, 32, PropModeAppend, (unsigned char *)&data,
							1);
...
setup_compatibility_props (ScreenInfo * scr) 
{
		...
		set_multi32bit_property (scr->Root, _XA_NET_SUPPORTED, XA_ATOM, 29, _XA_NET_SUPPORTED, _XA_NET_CLIENT_LIST,
								 _XA_NET_CLIENT_LIST_STACKING, _XA_NET_NUMBER_OF_DESKTOPS, _XA_NET_DESKTOP_GEOMETRY,
								 _XA_NET_DESKTOP_VIEWPORT, _XA_NET_CURRENT_DESKTOP, _XA_NET_ACTIVE_WINDOW,
		...
		
	{
			...
			
		{
			...
			read_as_property (wmprops->selection_window, _AS_STYLE, &(wmprops->as_styles_size),
							  &(wmprops->as_styles_version), &(wmprops->as_styles_data));
		...
		
	{
			...
			
		{
			...
			read_as_property (wmprops->selection_window, _AS_VISUAL, &(wmprops->as_visual_size),
							  &(wmprops->as_visual_version), &(wmprops->as_visual_data));
		...
		
	{
			...
			
		{
			...
			read_as_property (wmprops->selection_window, _AS_TBAR_PROPS, &(wmprops->as_tbar_props_size),
							  &(wmprops->as_tbar_props_version), &(wmprops->as_tbar_props_data));
...
read_root_wmprops (ASWMProps * wmprops, Bool do_cleanup) 
{
		...
		
	{
			...
			
		{
			...
			if (get_hash_item (wmprop_root_handlers, (ASHashableValue) all_props[props_num], &hdata.vptr) ==
				 ASH_Success)
...
read_volitile_wmprops (ASWMProps * wmprops) 
{
		...
		
	{
			...
			
		{
			...
			if (get_hash_item (wmprop_volitile_handlers, (ASHashableValue) all_props[props_num], &hdata.vptr) ==
				 ASH_Success)
...
set_desktop_num_prop (ASWMProps * wmprops, INT32 new_desk, Window vroot, Bool add) 
{
		...
		
	{
		...
		set_32bit_proplist (wmprops->scr->Root, _XA_NET_VIRTUAL_ROOTS, XA_WINDOW,
...
 Bool  set_current_viewport_prop (ASWMProps * wmprops, CARD32 vx, CARD32 vy, Bool normal) 
{
		...
		
	{
			...
			
		{
			...
			set_32bit_proplist (wmprops->scr->Root, _XA_NET_DESKTOP_VIEWPORT, XA_CARDINAL,
								 &(wmprops->desktop_viewport[0]), wmprops->desktop_viewports_num * 2);
...
set_stacking_order (ASWMProps * wmprops, Window * list, int nclients) 
{
		...
		
	{
		...
		{
			set_32bit_proplist (wmprops->scr->Root, _XA_NET_CLIENT_LIST_STACKING, XA_WINDOW, (CARD32 *) list,
								 nclients);
	...
	WMPropClass  handle_wmprop_event (ASWMProps * wmprops, XEvent * event) 
{
			...
			
		{
			...
			{
				if (get_hash_item (wmprop_volitile_handlers, AS_HASHABLE (event->xproperty.atom), &hdata.vptr) !=
					 ASH_Success)
					hdata.vptr = NULL;
			} else if (event->xproperty.window == wmprops->scr->Root)
				if (get_hash_item (wmprop_root_handlers, AS_HASHABLE (event->xproperty.atom), &hdata.vptr) !=
					 ASH_Success)
